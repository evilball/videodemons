...
$torrent_free	= request_var('torrent_free', 0);
$torrent_free > 100 || $torrent_free < 0 ? $torrent_free=0 : '';

$torrent_reqratio	= floatval(request_var('torrent_reqratio', 0.000));
$torrent_reqratio > 999.999 || $torrent_reqratio < 0.000 ? $torrent_reqratio=0.000 : '';

$torrent_requpload	= substr(request_var('torrent_requpload', 0), 0, 20);
$torrent_requpload < 0 ? $torrent_requpload=0 : '';

$posting_page=1;
!$config['ppkbb_poll_options'][0] ? $config['ppkbb_poll_options'][1]=0 : '';
$torrents_attach=$forum_data=$array_addfields=$torrents_remote=$torrents_hashes=array();
$dt=time();
...
if(@$post_data['forumas']==2)
{
	header('Location: '.append_sid('index.'.$phpEx));
	exit();
}
$forum_data['astracker']=@$post_data['forumas']==1 && $post_data['forum_id'] && ($mode=='post' || ($mode=='edit' && $post_id==$post_data['topic_first_post_id'])) ? 1 : 0;
$forum_data['image']=@$post_data['forum_image'] ? $post_data['forum_image'] : 0;
$is_cansetfree=$is_cansetreqratioupload=0;
if(isset($post_data['forum_addfields']) && $post_data['forum_addfields'])
{
	$torrent_addfield=get_torrent_addfield();
	$array_addfields=get_forum_addfields($post_data['forum_addfields'], $forum_data['astracker']);
}
...
if($forum_data['astracker'] && $mode=='post' && !$preview && !$submit && $config['ppkbb_poll_options'][0] && !$config['ppkbb_poll_options'][1])
{
	$post_data['poll_title']=$user->lang['VOTING_TORRENT'];
	$post_data['poll_option_text']=implode("\n", $user->lang['VOTING_TORRENT_VALS']);
	$post_data['poll_options']=$user->lang['VOTING_TORRENT_VALS'];
...
$is_canvote=$auth->acl_get('f_poll', $forum_id) || $config['ppkbb_poll_options'][0]==1 ? 1 : 0;

if($forum_data['astracker'])
{
	$ppkbb_max_torrents=1;
	$user->lang['ADD_ATTACHMENT']=$user->lang['TRACKER_ADD_ATTACHMENT'];
	$user->lang['ADD_ATTACHMENT_EXPLAIN']=sprintf($user->lang['TRACKER_ADD_ATTACHMENT_EXPLAIN'], $ppkbb_max_torrents, $ppkbb_max_torrents, $config['ppkbb_max_posters'][1], $config['ppkbb_max_posters'][0], $config['ppkbb_max_screenshots'][1], $config['ppkbb_max_screenshots'][0]);
	//$image_as_poster=request_var('as_poster', 0);
}
...
if ($post_data['post_attachment'] && !$submit && !$refresh && !$preview && $mode == 'edit')
{
	...
	$sql = 'SELECT attach_id, is_orphan, attach_comment, real_filename, extension, download_count, filesize, i_poster, i_external, i_width, i_height
...
{
	$is_cansetfree = $auth->acl_get('u_canfreetorrents') && $auth->acl_get('f_canfreetorrents', $forum_id) ? 1 : 0;
	$is_cansetreqratioupload = $auth->acl_get('u_cansetrequpratio') && $auth->acl_get('f_cansetrequpratio', $forum_id) ? 1 : 0;
	...
	if($config['ppkbb_max_extposters'][0])
	{
		...
		{
			$config['ppkbb_max_extposters'][0]=0;
		}
	}
	if($config['ppkbb_max_extscreenshots'][0])
	{
		...
		{
			$config['ppkbb_max_extscreenshots'][0]=0;
		}
	}
}
...
if ($submit || $preview || $refresh)
{
	if($mode == 'post' || ($mode == 'edit' && $post_id == $post_data['topic_first_post_id']))
	{
		...
		if($forum_data['astracker'])
		{
			if($config['ppkbb_max_extposters'][0] || $config['ppkbb_max_extscreenshots'][0])
			{
				include_once($phpbb_root_path.'tracker/include/posting_extimages_add1.'.$phpEx);
			}
		}
	}
	...
	if($forum_data['astracker'] && ($mode=='post' || ($mode=='edit' && $post_id==$post_data['topic_first_post_id'])))
	{
		if($config['ppkbb_poll_options'][1]==0)
		{
			$poll_delete=false;
		}
	}
	...
	else
	{
		if($forum_data['astracker'] && ($mode=='post' || ($mode=='edit' && $post_id==$post_data['topic_first_post_id'])))
		{
			if($config['ppkbb_poll_options'][0]!=0)
			{
				if($config['ppkbb_poll_options'][1]!=1)
				{
					$_REQUEST['poll_title']=$user->lang['VOTING_TORRENT'];
					$_REQUEST['poll_option_text']=implode("\n", $user->lang['VOTING_TORRENT_VALS']);
					$_REQUEST['poll_max_options']=1;
					$_REQUEST['poll_length']=0;
				}
			}
			else
			{
				$_REQUEST['poll_title']='';
				$_REQUEST['poll_option_text']='';
				$_REQUEST['poll_max_options']=0;
				$_REQUEST['poll_length']=0;
			}
		}
	...
	{
		if($config['ppkbb_max_mfu'] && $f > $config['ppkbb_max_mfu'])
		{
			break;
		}
	...
	if($forum_data['astracker'] && ($mode=='post' || ($mode=='edit' && $post_id==$post_data['topic_first_post_id'])))
	{
		...
		if($submit)
		{
			$post_data['topic_type'] == POST_GLOBAL ? $post_data['topic_type'] = POST_ANNOUNCE : '';
			$forum_data['valid_tracker_torrents']=$forum_data['valid_tracker_posters']=$forum_data['valid_tracker_screenshots']=0;
			$image_ext=array('jpg', 'jpeg', 'gif', 'png');
			foreach($message_parser->attachment_data as $k => $v)
			{
				if($v['extension']=='torrent')
				{
					$forum_data['valid_tracker_torrents']+=1;
				}
				else if(in_array($v['extension'], $image_ext))
				{
					if($v['i_poster']==1)
					{
						$forum_data['valid_tracker_posters']+=1;
					}
					else
					{
						$forum_data['valid_tracker_screenshots']+=1;
					}
				}
			}
			if(!$auth->acl_gets('a_', 'm_') && !$auth->acl_get('m_', $forum_id))
			{
				if($forum_data['valid_tracker_torrents'] > $ppkbb_max_torrents)
				{
					$t_attachments=0;
					$error['TMT']=$user->lang['MESS_TOOMANY_TORRENTS'];
				}
				if($forum_data['valid_tracker_torrents'] < $ppkbb_max_torrents)
				{
					$t_attachments=0;
					$error['TLT']=$user->lang['MESS_TOOMANY2_TORRENTS'];
				}
				if($config['ppkbb_max_posters'][1] && $forum_data['valid_tracker_posters'] > $config['ppkbb_max_posters'][1])
				{
					$t_attachments=0;
					$error['TMP']=$user->lang['MESS_TOOMANY_POSTERS'];
				}
				if($forum_data['valid_tracker_posters'] < $config['ppkbb_max_posters'][0])
				{
					$t_attachments=0;
					$error['TLP']=$user->lang['MESS_TOOMANY2_POSTERS'];
				}
				if($config['ppkbb_max_screenshots'][1] && $forum_data['valid_tracker_screenshots'] > $config['ppkbb_max_screenshots'][1])
				{
					$t_attachments=0;
					$error['TMS']=$user->lang['MESS_TOOMANY_SCREENSHOTS'];
				}
				if($forum_data['valid_tracker_screenshots'] < $config['ppkbb_max_screenshots'][0])
				{
					$t_attachments=0;
					$error['TLS']=$user->lang['MESS_TOOMANY2_SCREENSHOTS'];
				}
			}
		}
	}
	...
	if (!sizeof($error) && $submit)
	{
		...
		if ($submit)
		{
			...
			if($mode == 'post' || ($mode == 'edit' && $post_id == $post_data['topic_first_post_id']))
			{
				if($forum_data['astracker'] && ($config['ppkbb_max_extposters'][0] || $config['ppkbb_max_extscreenshots'][0]))
				{
					include_once($phpbb_root_path.'tracker/include/posting_extimages_add2.'.$phpEx);
				}
			}
			if($forum_data['astracker'])
			{
				if($config['ppkbb_chat_enable'] && $config['ppkbb_chat_bot'][0] && $mode=='post')
				{
					if($config['ppkbb_chat_botforums'] && (($config['ppkbb_chat_botforums_trueexclude'] && in_array($forum_id, $config['ppkbb_chat_botforums']))) || (!$config['ppkbb_chat_botforums_trueexclude'] && !in_array($forum_id, $config['ppkbb_chat_botforums'])))
					{
						$config['ppkbb_chat_bot'][0]=0;
					}
					else
					{
						$chat_redirect_url=preg_replace('/sid=([a-zA-Z0-9]{32})(&amp;)?/', '', $redirect_url);
						$chat_message=sprintf($user->lang['CHAT_BOT_MESSAGE'], html_entity_decode($config['ppkbb_chat_enable_bbcodes'] ? ($config['ppkbb_chat_enable_bbcodes'] ? "[url={$chat_redirect_url}]" : '').$post_data['post_subject'].($config['ppkbb_chat_enable_bbcodes'] ? "[/url]" : '') : $post_data['post_subject']), $post_data['forum_name']);
						$chat_message=utf8_strlen($chat_message) < $config['ppkbb_chat_messlength'] ? $chat_message : utf8_substr(sprintf($user->lang['CHAT_BOT_MESSAGE'], $post_data['post_subject'], $post_data['forum_name']), 0, $config['ppkbb_chat_messlength']);
						$result=$db->sql_query("INSERT INTO ".PPKCHAT_MESSAGES_TABLE." (message, user_id, date, rights, room, username, user_color, user_ip, forum_id) VALUES ('".$db->sql_escape($chat_message)."', '0', '{$dt}', '', '0', '".$db->sql_escape(isset($user->lang[$config['ppkbb_chat_bot'][1]]) ? $user->lang[$config['ppkbb_chat_bot'][1]] : $config['ppkbb_chat_bot'][1])."', '', INET_ATON('".$db->sql_escape($user->ip)."'), '{$forum_id}')");
					}
				}
			}
			...
			if(($mode == 'edit' && $post_id == $post_data['topic_first_post_id']) || $mode == 'post')
			{
				...
				$perm_show_unshow = $auth->acl_get('u_canusefpep') && $auth->acl_get('f_canusefpep', $forum_id) ? true : false;
			...
			else
			{
				...
				if($forum_data['astracker']/* && $mode!='reply'*/)
				{
					$torrent_url=$torrent_id='';
					foreach($message_parser->attachment_data as $torrent_data)
					{
						if($torrent_data['extension']=='torrent')
						{
							$torrent_id=intval($torrent_data['attach_id']);
							$torrent_url=append_sid("{$phpbb_root_path}download/file.$phpEx", 'id=' . $torrent_id, true, ($torrent_data['is_orphan']) ? $user->session_id : false);
							break;
						}
					}
					if($torrent_id)
					{
						$redirect_time=10;
						$message.='<br /><br />' . sprintf(($mode == 'edit' ? $user->lang['VIEW_EDITED_TORRENT'] : $user->lang['VIEW_TORRENT']), '<a href="' . $torrent_url . '">', '</a>');
						$db->sql_query("SELECT info_hash FROM ".TRACKER_TORRENTS_TABLE." WHERE id='{$torrent_id}'");
						$torrent_hash=$db->sql_fetchfield('info_hash');
						$torrents_hashes=array($torrent_id=>$torrent_hash);
						if($config['ppkbb_tcenable_rannounces'][0] && $config['ppkbb_tcenable_rannounces'][4])
						{
						...
						include_once($phpbb_root_path.'tracker/include/posting_add_cron.'.$phpEx);
					}
				}

				meta_refresh($redirect_time, $redirect_url);
			}
...
if (!sizeof($error) && $preview)
{
	...
	if (sizeof($message_parser->attachment_data))
	{
		...
		if($forum_data['astracker'])
		{
			...
			foreach ($attachment_data as $i => $attachment)
			{
				...
				{
					$torrents_attach['torrent'][]=$attachment;
					$attachment_data[$i]['skip_attachment']=1;
				}
				else if(in_array($attachment['extension'], $image_ext) && !$attachment['i_external'])
				{
					if($attachment['i_poster']==1)
					{
						$torrents_attach['poster'][]=$attachment;
					}
					else
					{
						$torrents_attach['screenshot'][]=$attachment;
					}
					$attachment_data[$i]['skip_attachment']=1;
				}
				$attachment['i_external'] ? $attachment_data[$i]['skip_attachment']=1 : '';
			}
			reset($attachment_data);
		}
	...
	{
		$template->assign_vars(array(
			'S_HAS_TRACKER_TORRENT'	=> isset($torrents_attach['torrent']) && sizeof($torrents_attach['torrent']) ? true : false,
			'S_HAS_TRACKER_POSTER'	=> isset($torrents_attach['poster']) && sizeof($torrents_attach['poster']) ? true : false,
			'S_HAS_TRACKER_SCREENSHOT'	=> isset($torrents_attach['screenshot']) && sizeof($torrents_attach['screenshot']) ? true : false,
			'FORUM_IMAGE'				=> $forum_data['image'] ? $phpbb_root_path.$forum_data['image'] : '',
			'POSTERS_TABLE_WIDTH' => $forum_data['astracker'] ? $config['ppkbb_torrblock_width'][8]+$config['ppkbb_torrblock_width'][1] : '',
			'SCREENSHOTS_TABLE_WIDTH' => $forum_data['astracker'] ? $config['ppkbb_torrblock_width'][10]+$config['ppkbb_torrblock_width'][1] : '',
...
if($mode=='quote' && $post_data['forumas']==1 && $config['ppkbb_disable_fpquote'] && $post_data['post_attachment'])
{
	$result=$db->sql_query("SELECT post_msg_id FROM ".ATTACHMENTS_TABLE." WHERE post_msg_id = '{$post_id}' AND extension = 'torrent' LIMIT 1");
	$is_tracker=$db->sql_fetchfield('post_msg_id');
	if($is_tracker)
	{
...
if($forum_data['astracker'])
{
	$form_enctype = (@ini_get('file_uploads') != '0'
				...
				&& (
					($auth->acl_get('u_attachfonly') && ($mode=='post' || ($mode=='edit' && $post_id==$post_data['topic_first_post_id'])))
					|| ($auth->acl_get('f_attachfonly', $forum_id) && ($mode=='post' || ($mode=='edit' && $post_id==$post_data['topic_first_post_id']))
					)
				)
...
$addfields_type_switch=!$submit && !$preview/* && !$config['ppkbb_addfields_type'][0]*/ && sizeof($array_addfields) && $mode=='post' && !request_var('message', '', true) ? true : false;
$addfields_inform_place=/*!$config['ppkbb_addfields_type'][0] && */$config['ppkbb_addfields_type'][1] ? true : false;

// Start assigning vars for main posting page ...
$template->assign_vars(array(
	'S_FIRST_POST_SHOW_ALLOWED'		=> $first_post_show_allowed  && $auth->acl_get('u_canusefpep') && $auth->acl_get('f_canusefpep', $forum_id) ? true : false,
	'S_FIRST_POST_SHOW_CHECKED'		=> ($first_post_show_checked) ? ' checked="checked"' : '',
	'S_DISABLE_POLLTAB'			=>$forum_data['astracker'] ? (($config['ppkbb_poll_options'][0]!=0 && $config['ppkbb_poll_options'][1]!=0) ? false : true) : false,
	'SUBJECT_TEXTLENGTH'		=> $config['ppkbb_subject_textlength'],
	'SUBJECT_TEXTLENGTH2'		=> $config['ppkbb_subject_textlength']+4,
	'TRACKER_ANNOUNCE_URL'		=> $forum_data['astracker'] ? sprintf($user->lang['TRACKER_ANNOUNCE_URL'], $tracker_url.$config['ppkbb_announce_url']) : '',
	'ADDFIELDS_DESCR'					=> sizeof($array_addfields) ? $torrent_addfield['TRACKER_FORUM_ADDFIELDS_DESCR'] : '',
	'ADDFIELDS_TYPE'					=> /*!$config['ppkbb_addfields_type'][0] && */$config['ppkbb_addfields_type'][1] ? true : (/*$config['ppkbb_addfields_type'][0] ? true : */false),
	'ADDFIELDS_TYPE_SWITCH'					=> /*!$config['ppkbb_addfields_type'][0] && */$config['ppkbb_addfields_type'][1] ? false : $addfields_type_switch,
	'ADDFIELDS_INFORM_PLACE'					=> $addfields_inform_place,
	'S_TORRENT_FREE'			=>$forum_data['astracker'] ? $is_cansetfree : 0,

	'S_TORRENT_REQUPRATIO'			=>$forum_data['astracker'] ? $is_cansetreqratioupload : 0,
	'S_SHOW_ANNWARN' => $forum_data['astracker'] && $config['ppkbb_tfile_annreplace'][0] ? true : false,
	'S_ANNWARN_CHECKED' => $forum_data['astracker'] && $config['ppkbb_tfile_annreplace'][0] && $config['ppkbb_tfile_annreplace'][2] ? true : false,
	//START MFU
	'S_IS_TRACKER' => $forum_data['astracker'] ? true : false,
	'S_MAX_MFU' => $config['ppkbb_max_mfu'] ? $config['ppkbb_max_mfu'] : false,
	'L_MAX_MFU' => $config['ppkbb_max_mfu'] ? sprintf($user->lang['PPKBB_MAX_MFU'], $config['ppkbb_max_mfu']) : false,
	//END MFU
	'L_POST_A'					=> $page_title,
...
if($forum_data['astracker'])
{
	include_once($phpbb_root_path.'tracker/include/posting_add2.'.$phpEx);
}
...
$allowed = ($auth->acl_get('f_attach', $forum_id) || ($forum_data['astracker'] && $auth->acl_get('f_attachfonly', $forum_id) && ($mode=='post' || $mode=='edit'))) && ($auth->acl_get('u_attach') || ($forum_data['astracker'] && $auth->acl_get('u_attachfonly', $forum_id) && ($mode=='post' || $mode=='edit'))) &&
		$config['allow_attachments'] &&
		$form_enctype;
