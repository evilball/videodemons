...
*
* @package ppkBB3cker
* @version $Id: announce.php 1.000 2008-10-05 12:31:00 PPK $
...
$passkey=STRIP ? @$_GET['passkey'] : mysql_real_escape_string(@$_GET['passkey'], $c);
if(!$passkey)
{
...
if($config['ppkbb_tczones_enable'][1])
{
	include($tincludedir.'tvalidip.'.$phpEx);
}

$sql="SELECT tt.id, tt.seeders, tt.leechers, tt.times_completed, tt.added, tt.free, tt.size, tt.upload, tt.forb, tt.req_ratio, tt.req_upload, tt.poster_id, tt.unreg, tt.tsl_speed, u.user_id, u.user_uploaded, u.user_downloaded, u.user_permissions, u.user_bonus, u.user_type, u.user_tracker_data, s.finished, s.bonus_count FROM ".USERS_TABLE." u LEFT JOIN " . TRACKER_TORRENTS_TABLE . " tt ON (tt.info_hash='{$info_hash}') LEFT JOIN ".TRACKER_SNATCHED_TABLE." s ON (tt.id=s.torrent AND u.user_id=s.userid".($config['ppkbb_tcguests_enabled'][0] ? " AND s.guests='0'" : '').") WHERE u.user_passkey='{$passkey}' AND u.user_type IN (" . USER_NORMAL . ', ' . USER_FOUNDER . ") LIMIT 1";
...
$user['forb'] < 1 || ($user['forb'] > 0 && $userid==$user['poster_id'] && in_array($user['forb'], $config['ppkbb_tcauthor_candown'])) ? '' : err("Forbidden or Blocked torrent");

$selfwhere="tp.torrent='{$torrentid}' AND tp.userid='{$userid}'".($config['ppkbb_tcguests_enabled'][0] ? " AND tp.guests='0'" : '');
...
if(($seeder && $config['ppkbb_tcmax_seed'] && !$rights[7]) || (!$seeder && ($config['ppkbb_tcmax_leech'] && !$rights[8]) || ($config['ppkbb_tcmaxleech_restr'] && !$rights[2])))
{
	include($tincludedir.'tmaxsl.'.$phpEx);
}
...
/*if($config['ppkbb_tcenable_rannounces'][0] && $config['ppkbb_tcenable_rannounces'][8])
{
	include($tincludedir.'trannounces.'.$phpEx);
}*/
