...
*
* @package ppkBB3cker
* @version $Id: search_add_cron.php 1.000 2010-07-11 19:03:00 PPK $
...
{
	$sql="INSERT INTO ".TRACKER_CRON_TABLE."(type, data, forum_id, added) VALUES('t_clean', '".$db->sql_escape(serialize($torrents_cleanup))."', '0', '{$dt}')";
	$db->sql_query($sql);
...
{
	$sql="INSERT INTO ".TRACKER_CRON_TABLE."(type, data, forum_id, added) VALUES('update_tstatus', '".$db->sql_escape(serialize($update_tstatus))."', '0', '{$dt}')";
	$db->sql_query($sql);
	$cron_id=$db->sql_nextid();
	if($cron_id)
	{
