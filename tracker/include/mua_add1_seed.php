...
*
* @package ppkBB3cker
* @version $Id: mua_add1_seed.php 1.000 2010-10-29 14:16:00 PPK $
...
$sql='SELECT p.*, INET_NTOA(p.ip) ip, t.forum_id, t.topic_id, t.topic_title, t.topic_first_post_id, s.prev_action, s.uploadoffset, s.downloadoffset, tt.size, tt.unreg FROM '. TRACKER_PEERS_TABLE .' p LEFT JOIN '.ATTACHMENTS_TABLE." a ON (p.torrent=a.attach_id) LEFT JOIN ".TOPICS_TABLE." t ON (a.topic_id=t.topic_id) LEFT JOIN ".TRACKER_SNATCHED_TABLE." s ON (s.torrent=p.torrent AND s.userid=p.userid) LEFT JOIN ".TRACKER_TORRENTS_TABLE." tt ON (p.torrent=tt.id) WHERE p.userid='{$mua_user_id}' AND p.guests='0' AND p.seeder='1'".(sizeof($ex_fid_ary) ? " AND tt.forum_id NOT IN('".implode("', '", $ex_fid_ary)."')" : '').($mua_limit ? " LIMIT {$mua_limit}" : '');// ORDER BY $sql_addon";
