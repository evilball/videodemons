...
*
* @package ppkBB3cker
* @version $Id: mua_add1_fromthanks.php 1.000 2010-10-29 14:14:00 PPK $
...
$sql="SELECT t.id thanks_id, t.tadded, u.username, u.user_id, u.user_colour, tt.forum_id, tt.topic_id, tt.topic_title, tt.topic_first_post_id, s.*, ttt.size, ttt.unreg FROM ".TRACKER_THANKS_TABLE." t LEFT JOIN ".USERS_TABLE." u ON (t.to_user=u.user_id) LEFT JOIN ".ATTACHMENTS_TABLE." a ON (t.torrent_id=a.attach_id) LEFT JOIN ".TOPICS_TABLE." tt ON (t.post_id=tt.topic_first_post_id) LEFT JOIN ".TRACKER_SNATCHED_TABLE." s ON (t.user_id=s.userid AND t.torrent_id=s.torrent) LEFT JOIN ".TRACKER_TORRENTS_TABLE." ttt ON (t.torrent_id=ttt.id) WHERE t.user_id='{$mua_user_id}'".(sizeof($ex_fid_ary) ? " AND ttt.forum_id NOT IN('".implode("', '", $ex_fid_ary)."')" : '').($mua_limit ? " LIMIT {$mua_limit}" : '');// ORDER BY $sql_addon";
