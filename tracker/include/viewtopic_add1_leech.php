...
*
* @package ppkBB3cker
* @version $Id: viewtopic_add1_leech.php 1.000 2009-02-07 11:26:00 PPK $
...
{
	$sql_addon="p.uploadoffset/(p.last_action-p.prev_action) $opt_sort";
}
else if($opt_param=='downspeed')
{
	$sql_addon="p.downloadoffset/(p.last_action-p.prev_action) $opt_sort";
}
else
{
...
$sql='SELECT p.*, INET_NTOA(p.ip) ip, u.username, u.user_id, u.user_colour, s.prev_action, s.uploadoffset, s.downloadoffset FROM '. TRACKER_PEERS_TABLE ." p LEFT JOIN ". USERS_TABLE ." u ON (p.userid=u.user_id) LEFT JOIN ".TRACKER_SNATCHED_TABLE." s ON (s.torrent=p.torrent AND s.userid=p.userid) WHERE p.torrent='{$torrent_id}' AND p.seeder='0'".($mua_limit ? " LIMIT {$mua_limit}" : '');// ORDER BY $sql_addon";
...
if($config['ppkbb_tcenable_rannounces'][0])
{
	$template->assign_vars(array(
		'S_REMLEECHS' => isset($torrents[$torrent_id]['rem_leechers']) ? sprintf($user->lang['TRACKER_REMLEECHS'], $torrents[$torrent_id]['rem_leechers']) : false,
		)
	);
}
