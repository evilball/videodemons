...
*
* @package ppkBB3cker
* @version $Id: mua_add1_leave.php 1.000 2010-10-29 14:15:00 PPK $
...
$sql='SELECT s.*, t.forum_id, t.topic_id, t.topic_title, t.topic_first_post_id, tt.size, tt.unreg FROM '. TRACKER_SNATCHED_TABLE .' s LEFT JOIN '.TRACKER_PEERS_TABLE." p ON (p.userid=s.userid AND p.torrent=s.torrent) LEFT JOIN ".ATTACHMENTS_TABLE." a ON (s.torrent=a.attach_id) LEFT JOIN ".TOPICS_TABLE." t ON (a.topic_id=t.topic_id) LEFT JOIN ".TRACKER_TORRENTS_TABLE." tt ON (s.torrent=tt.id) WHERE s.userid='{$mua_user_id}' AND s.guests='0' AND ISNULL(p.torrent) AND s.finished!='0'".(sizeof($ex_fid_ary) ? " AND tt.forum_id NOT IN('".implode("', '", $ex_fid_ary)."')" : '').($mua_limit ? " LIMIT {$mua_limit}" : '');// ORDER BY $sql_addon";
