...
class acp_main
{
	...
	function main($id, $mode)
	{
				...
				switch ($action)
				{
					...
					case 'stats':
						...
						$sql = 'SELECT SUM(forum_topics) AS stat
							...
							WHERE forumas=1';
						$result = $db->sql_query($sql);
						$num_torrents=$db->sql_fetchfield('stat');
						$db->sql_freeresult($result);
						set_config('num_torrents', (int) $num_torrents, true);
						...
						$sql = 'SELECT SUM(forum_posts)-SUM(forum_topics) AS stat
							...
							WHERE forumas=1';
						$result = $db->sql_query($sql);
						$num_comments=$db->sql_fetchfield('stat');
						$db->sql_freeresult($result);
						set_config('num_comments', (int) $num_comments, true);

						//purge_tracker_config();
						...
						$sql = 'SELECT COUNT(attach_id) as stat
							...
							WHERE is_orphan = 0 AND i_external = 0';
						$result = $db->sql_query($sql);
					...
					case 'purge_sessions':
						...
						$reinsert_ary = array(
								...
								'session_topic_id' => $user->page['topic'],
		...
		//if(request_var('update_tracker_version', 0))
		//{
			include_once("{$phpbb_root_path}/tracker/include/includes/acp/acp_main_add1_update.{$phpEx}");
			if(implode('_', $config['ppkbb_ppkbb3cker_version'])!=$ppkbb3cker_current_version)
			{
				set_tracker_config('ppkbb_ppkbb3cker_version', $ppkbb3cker_current_version);
				set_tracker_config('ppkbb_ppkbb3cker_new', '');
				set_config('ppkbb_newversion_lastcheck', 0, 1);
				purge_tracker_config(true);
				$redirect_url = append_sid("{$phpbb_admin_path}index.$phpEx");
				meta_refresh(3, $redirect_url);
				trigger_error($user->lang['PPKBB_VERSION_UPDATED'] . '<br /><br />' . sprintf($user->lang['RETURN_INDEX'], '<a href="' . $redirect_url . '">', '</a>'));
			}
		//}
		...
		$ppkbb3cker_update=array();
		if($config['ppkbb_ppkbb3cker_version'] && time()-$config['ppkbb_newversion_lastcheck']>86400*1)
		{
			$nv=get_remote_file('www.ppkbb3cker.ru', '/', "check_new_version.php?v=".urlencode($config['ppkbb_ppkbb3cker_version'][0])."&r=".$config['ppkbb_ppkbb3cker_version'][1]."&u=".$config['ppkbb_ppkbb3cker_version'][2]."&i=".urlencode(@$_SERVER['SERVER_ADDR'])."&n=".urlencode(@$_SERVER['SERVER_NAME'])."&h=".urlencode($config['server_name']), $errstr, $errno);
			if($nv)
			{
				$nv=my_split_config($nv, 3, array('strval', 'my_int_val', 'floatval'), '_');
				if($config['ppkbb_ppkbb3cker_version'][0]==$nv[0])
				{
					$config['ppkbb_ppkbb3cker_version'][1]<$nv[1] ? $ppkbb3cker_update[]=sprintf($user->lang['PPKBB3CKER_NEWREVISION'], $config['ppkbb_ppkbb3cker_version'][1], $nv[1]) : '';
					$config['ppkbb_ppkbb3cker_version'][2]<$nv[2] ? $ppkbb3cker_update[]=sprintf($user->lang['PPKBB3CKER_NEWUPDATE'], $config['ppkbb_ppkbb3cker_version'][2], $nv[2]) : '';
					sizeof($ppkbb3cker_update) ? $ppkbb3cker_update[]='<a href="http://www.ppkbb3cker.ru/" target="_blank">www.ppkbb3cker.ru</a>' : '';
					set_config('ppkbb_newversion_lastcheck', time(), 1);
					set_tracker_config('ppkbb_ppkbb3cker_new', (sizeof($ppkbb3cker_update) ? implode('<br />', $ppkbb3cker_update) : ''));
					purge_tracker_config(true);
				}
			}
		}
		$template->assign_vars(array(
			...
			'PPKBB3CKER_VERSION' => $config['ppkbb_ppkbb3cker_version'] ? sprintf($user->lang['ADM_PPKBB3CKER_VERSION'], $config['ppkbb_ppkbb3cker_version'][0], $config['ppkbb_ppkbb3cker_version'][1], $config['ppkbb_ppkbb3cker_version'][2]) : '',
			'PPKBB3CKER_UPDATE' => sizeof($ppkbb3cker_update) ? implode('<br />', $ppkbb3cker_update) : ($config['ppkbb_ppkbb3cker_new'] ? $config['ppkbb_ppkbb3cker_new'] : false),
