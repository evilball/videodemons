...
class acp_users
{
	...
	function main($id, $mode)
	{
		...
		switch ($mode)
		{
			...
			break;
			case 'torrents':
				include($phpbb_root_path . 'tracker/include/includes/acp/acp_users_add_torrents.' . $phpEx);
			break;

			case 'tracker':
				include($phpbb_root_path . 'tracker/include/includes/acp/acp_users_add_tracker.' . $phpEx);
			break;

			case 'rtracker':
				include($phpbb_root_path . 'tracker/include/includes/acp/acp_users_add_rtracker.' . $phpEx);
			break;
			case 'prefs':
			...
			case 'groups':
				...
				while ($row = $db->sql_fetchrow($result))
				{
					...
					$s_group_options .= '<option' . (($row['group_type'] == GROUP_SPECIAL) ? ' class="sep"' : '') . ' value="' . $row['group_id'] . '">' . (($row['group_type'] == GROUP_SPECIAL) || @$user->lang['G_' . $row['group_name']] ? $user->lang['G_' . $row['group_name']] : $row['group_name']) . '</option>';
				}
				$db->sql_freeresult($result);
				...
				foreach ($group_data as $group_type => $data_ary)
				{
					...
					foreach ($data_ary as $data)
					{
						$template->assign_block_vars('group', array(
							...
							'GROUP_NAME'		=> ($group_type == 'special') || @$user->lang['G_' . $data['group_name']] ? $user->lang['G_' . $data['group_name']] : $data['group_name'],
