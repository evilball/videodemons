			...
			switch ($action)
			{
				...
				case 'add':
					...
					$forum_data += array(
						...
						'forum_first_post_show'	=> request_var('forum_first_post_show', 0),
						'forum_addfields'	=> request_var('forum_addfields', 0),
						'forumas'	=> request_var('forumas', 0),
...
*/
		...
		switch ($action)
		{
			...
			case 'edit':
				...
				$template->assign_vars(array(
					...
					'S_FORUM_ASTRACKER'	=> $forum_data['forumas']==1 ? 1 : 0,
					'S_FORUM_ASCHAT' => $forum_data['forumas']==2 ? 1 : 0,
					'S_FORUM_ASFORUM' => !$forum_data['forumas'] ? 1 : 0,
					'S_ADDFIELDS_OPTIONS' => $af_selects,

					'FORUM_FIRST_POST_SHOW' => $forum_data['forum_first_post_show'],
	...
	*/
	function move_forum_content($from_id, $to_id, $sync = true)
	{
		...
		$table_ary = array(LOG_TABLE, POSTS_TABLE, TOPICS_TABLE, DRAFTS_TABLE, TOPICS_TRACK_TABLE, TRACKER_TORRENTS_TABLE);
	...
	*/
	function delete_forum_content($forum_id)
	{
		...
		$sql = 'SELECT COUNT(attach_id) as stat
			FROM ' . ATTACHMENTS_TABLE . ' WHERE i_external = 0';
		$result = $db->sql_query($sql);
		$row = $db->sql_fetchrow($result);
		$db->sql_freeresult($result);
