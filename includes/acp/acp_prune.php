...
class acp_prune
{
	...
	*/
	function prune_users($id, $mode)
	{
		...
		$find_uploaded = array('lt' => $user->lang['LESS_THAN'], 'eq' => $user->lang['EQUAL_TO'], 'gt' => $user->lang['MORE_THAN']);
		$s_find_uploaded = '';
		foreach ($find_uploaded as $key => $value)
		{
			$selected = ($key == 'eq') ? ' selected="selected"' : '';
			$s_find_uploaded .= '<option value="' . $key . '"' . $selected . '>' . $value . '</option>';
		}

		$find_downloaded = array('lt' => $user->lang['LESS_THAN'], 'eq' => $user->lang['EQUAL_TO'], 'gt' => $user->lang['MORE_THAN']);
		$s_find_downloaded = '';
		foreach ($find_downloaded as $key => $value)
		{
			$selected = ($key == 'eq') ? ' selected="selected"' : '';
			$s_find_downloaded .= '<option value="' . $key . '"' . $selected . '>' . $value . '</option>';
		}

		$template->assign_vars(array(
			'S_RATIO_OPTIONS'	=> $s_find_ratio,
			'S_UPLOADED_OPTIONS'	=> $s_find_uploaded,
			'S_DOWNLOADED_OPTIONS'	=> $s_find_downloaded,
			'U_ACTION'			=> $this->u_action,
	...
	*/
	function get_prune_users(&$user_ids, &$usernames)
	{
		...
		else
		{
			...
			$joined = request_var('joined', '');
			$active = request_var('active', '');
			$ratio_select = request_var('ratio_select', 'lt');
			$uploaded_select = request_var('uploaded_select', 'lt');
			$downloaded_select = request_var('downloaded_select', 'lt');

			$ratio = my_float_val(request_var('ratio', ''));
			$uploaded = get_size_value(request_var('uploadedv', 'b'), request_var('uploaded', 0));
			$downloaded = get_size_value(request_var('downloadedv', 'b'), request_var('downloaded', 0));
			$upv = request_var('uploaded', '');
			$downv = request_var('downloaded', '');
			...
			$sort_by_types = array('username', 'user_email', 'user_posts', 'user_regdate', 'user_lastvisit', 'user_uploaded', 'user_downloaded', 'user_ratio');
			...
			$where_sql .= ($ratio != 0.000) ? " AND user_uploaded/user_downloaded " . $key_match[$ratio_select] . ' ' . $ratio . ' ' : '';
			$where_sql .= ($upv!=='') ? " AND user_uploaded " . $key_match[$uploaded_select] . ' ' . $uploaded . ' ' : '';
			$where_sql .= ($downv!=='') ? " AND user_downloaded " . $key_match[$downloaded_select] . ' ' . $downloaded . ' ' : '';
		...
		$sql = 'SELECT user_id, username, user_uploaded/user_downloaded user_ratio
			FROM ' . USERS_TABLE . '
