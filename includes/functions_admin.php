...
*/
function make_forum_select($select_id = false, $ignore_id = false, $ignore_acl = false, $ignore_nonpost = false, $ignore_emptycat = true, $only_acl_post = false, $return_array = false, $only_trackers=0, $exclude_chat=0)
{
	global $db, $user, $auth;

	// This query is identical to the jumpbox one
	$sql = 'SELECT forum_id, forum_name, parent_id, forum_type, forum_flags, forum_options, left_id, right_id, forumas
		FROM ' . FORUMS_TABLE . '
		ORDER BY left_id ASC';
	...
	while ($row = $db->sql_fetchrow($result))
	{
		...
		(($only_trackers==1 && $row['forumas']!=1) || ($only_trackers==2 && $row['forumas']!=2) || ($exclude_chat && $row['forumas']==2)) ? $disabled=true : '';
		if (
		...
		else
		{
			...
			$forum_list .= '<option value="' . $row['forum_id'] . '"' . (($disabled || ($only_trackers==1 && $row['forumas']!=1) || ($only_trackers==2 && $row['forumas']!=2)) ? ' disabled="disabled" class="disabled-option"' : $selected) . '>' . $padding . $row['forum_name'] . '</option>';
		}
	}
	$db->sql_freeresult($result);
	unset($padding_store);
...
function group_select_options($group_id, $exclude_ids = false, $manage_founder = false)
{
	...
	while ($row = $db->sql_fetchrow($result))
	{
		...
		$s_group_options .= '<option' . (($row['group_type'] == GROUP_SPECIAL) ? ' class="sep"' : '') . ' value="' . $row['group_id'] . '"' . $selected . '>' . (($row['group_type'] == GROUP_SPECIAL) || @$user->lang['G_' . $row['group_name']] ? $user->lang['G_' . $row['group_name']] : $row['group_name']) . '</option>';
	}
	$db->sql_freeresult($result);
...
function move_topics($topic_ids, $forum_id, $auto_sync = true)
{
	...
	$result=$db->sql_query("SELECT forumas FROM ".FORUMS_TABLE." WHERE forum_id='{$forum_id}' LIMIT 1");
	$forum_aschat=$db->sql_fetchfield('forumas');
	if($forum_aschat==2)
	{
		trigger_error('NO_MOVE_TOPICS_TOCHAT');
	}
	...
	$table_ary = array(TOPICS_TABLE, POSTS_TABLE, LOG_TABLE, DRAFTS_TABLE, TOPICS_TRACK_TABLE, TRACKER_TORRENTS_TABLE);
...
function move_posts($post_ids, $topic_id, $auto_sync = true)
{
	...
	$result=$db->sql_query("SELECT forumas FROM ".FORUMS_TABLE." WHERE forum_id='{$forum_row['forum_id']}' LIMIT 1");
	$forum_aschat=$db->sql_fetchfield('forumas');
	if($forum_aschat==2)
	{
		trigger_error('NO_MOVE_POSTS_TOCHAT');
	}
	...
	$sql = 'UPDATE ' . TRACKER_TORRENTS_TABLE . '
		SET forum_id = ' . (int) $forum_row['forum_id'] . ", topic_id = $topic_id
		WHERE " . $db->sql_in_set('post_msg_id', $post_ids);
	$db->sql_query($sql);
...
function delete_topics($where_type, $where_ids, $auto_sync = true, $post_count_sync = true, $call_delete_posts = true)
{
	...
	$table_ary = array(BOOKMARKS_TABLE, TOPICS_TRACK_TABLE, TOPICS_POSTED_TABLE, POLL_VOTES_TABLE, POLL_OPTIONS_TABLE, TOPICS_WATCH_TABLE, TOPICS_TABLE, TRACKER_TORRENTS_TABLE);
...
/*		Possible code for splitting post deletion
	...
	$table_ary = array(POSTS_TABLE, REPORTS_TABLE, TRACKER_THANKS_TABLE);
	...
	$table_ary = array(TRACKER_TORRENTS_TABLE);
...
function delete_attachments($mode, $ids, $resync = true)
{
	...
	if($attach_ids)
	{
		...
		$sql = 'DELETE FROM ' . TRACKER_PEERS_TABLE . '
			WHERE ' . $db->sql_in_set('torrent', array_keys($attach_ids));
		$db->sql_query($sql);

		$sql = 'DELETE FROM ' . TRACKER_SNATCHED_TABLE . '
			WHERE ' . $db->sql_in_set('torrent', array_keys($attach_ids));
		$db->sql_query($sql);

		$sql = 'DELETE FROM ' . TRACKER_RANNOUNCES_TABLE . '
			WHERE ' . $db->sql_in_set('torrent', $attach_ids);
		$db->sql_query($sql);

		$sql = "DELETE FROM ".TRACKER_RTRACK_TABLE."
			WHERE torrent!='0' AND torrent IN('".implode("', '", $attach_ids)."')";
		$db->sql_query($sql);

		/*$sql = 'DELETE FROM ' . TRACKER_THANKS_TABLE . '
			WHERE ' . $db->sql_in_set('torrent_id', array_keys($attach_ids));
		$db->sql_query($sql);*/
	}
...
function view_log($mode, &$log, &$log_count, $limit = 0, $offset = 0, $forum_id = 0, $topic_id = 0, $user_id = 0, $limit_days = 0, $sort_by = 'l.log_time DESC', $keywords = '')
{
	...
	while ($row = $db->sql_fetchrow($result))
	{
		...
		$log[$i] = array(
			'chat'			=> $row['log_operation'] && preg_match('/^LOG_CHAT_/', $row['log_operation']) ? true : false,
			'id'				=> $row['log_id'],
