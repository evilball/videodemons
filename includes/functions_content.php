...
function gen_torrents_selects()
{
	global $user;
	$curr=request_var('show_torrents', 't');

	$torrents_sel_array=array(
		'all'=>$user->lang['DISPLAY_TORRENTS_ALL'],
		'withoutbad'=>$user->lang['DISPLAY_TORRENTS_WBAD'],
		'good'=>$user->lang['DISPLAY_TORRENTS_GOOD'],
		'dead'=>$user->lang['DISPLAY_TORRENTS_DEAD'],
		'noseed'=>$user->lang['DISPLAY_TORRENTS_NOSEED'],
		'bad'=>$user->lang['DISPLAY_TORRENTS_BAD'],
	);

	$torrents_select='<select name="show_torrents">';
	foreach($torrents_sel_array as $k => $v)
	{
		$torrents_select.='<option value="'.$k.'"'.($curr==$k ? ' selected="selected"' : '').'>'.$v.'</option>';
	}
	$torrents_select.='</select>';

	return $torrents_select;
}
...
function parse_attachments($forum_id, &$message, &$attachments, &$update_count, $preview = false)
{
	...
	{
		if (!sizeof($attachment) || $attachment['i_external'] || (isset($attachment['skip_attachment']) && $attachment['skip_attachment']))
		{
			if(/*$attachment['i_external'] || */(isset($attachment['skip_attachment']) && $attachment['skip_attachment']) && strtolower(trim($attachment['extension']))!='torrent')
			{
		...
		if (!$denied)
		{
			...
			{
				if (isset($attachment['thumbnail']) && $attachment['thumbnail'])
				{
			...
			switch ($display_cat)
			{
				...
				case ATTACHMENT_CATEGORY_IMAGE:
					...
					$download_link .= '&amp;mode=view'.'&amp;ext=.'.$attachment['extension'];
