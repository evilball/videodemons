...
{
	global $auth, $user, $template, $topic_type, $forum_data;
...
function upload_attachment($form_name, $forum_id, $local = false, $local_storage = '', $is_message = false, $local_filedata = false)
{
	...
	global $phpbb_root_path, $phpEx, $forum_data;

	$filedata = array(
		'error'	=> array()
	);

	if($form_name=='local' && (!$local_storage || !$local_filedata))
	{
		return false;
	}
	...
	if($forum_data['astracker'])
	{
		$config['img_create_thumbnail']=1;
		$config['img_max_width']=$config['ppkbb_tmax_imgwidth'];
		$config['img_max_height']=$config['ppkbb_tmax_imgheight'];
	}
	...
	if ($filedata['thumbnail'])
	{
		...
		if($forum_data['astracker'])
		{
			list($filedata['i_width'], $filedata['i_height'], )=@getimagesize($source);
		}
	}
...
{
	global $config, $forum_data;
	$forum_data['astracker'] ? $config['img_min_thumb_filesize']=$config['ppkbb_tmin_thumbsize'] : '';
	...
	list($new_width, $new_height) = $forum_data['astracker'] ? tracker_get_img_size_format($width, $height) : get_img_size_format($width, $height);
...
function posting_gen_inline_attachments(&$attachment_data)
{
	...
	if (sizeof($attachment_data))
	{
		...
		foreach ($attachment_data as $i => $attachment)
		{
			...
			if(@$attachment['i_external'] || @$attachment['skip_attachment'])
			{
			...
			else
			{
				...
				$s_inline_attachment_options .= '<option value="' . $i2 . '">' . utf8_basename($attachment['real_filename']) . '</option>';
...
{
	global $template, $config, $phpbb_root_path, $phpEx, $user, $auth, $forum_data;
	...
	if (sizeof($attachment_data))
	{
		...
		{
			$hidden = '';
			//$d_delete=$attach_row['extension']=='torrent' ? 1 : 0;
			$attach_row['real_filename'] = @$attach_row['i_external'] ? $attach_row['real_filename'] : utf8_basename($attach_row['real_filename']);
			...
			$download_link = (@$attach_row['i_external'] ? $attach_row['real_filename'] : append_sid("{$phpbb_root_path}download/file.$phpEx", 'mode=view&amp;id=' . (int) $attach_row['attach_id'], true, ($attach_row['is_orphan']) ? $user->session_id : false));

			$template->assign_block_vars('attach_row', array(
				//'DISABLE_DELETE'	=> $forum_data['astracker'] && $d_delete ? ' disabled="disabled"' : '',
				'SKIP_ATTACHMENT' => @$attach_row['i_external'] ? true : false,
...
function topic_review($topic_id, $forum_id, $mode = 'topic_review', $cur_post_id = 0, $show_quote_button = true)
{
	...
	for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
	{
		...
		if($config['ppkbb_disable_fpquote'] && $has_attachments && $end-1==$i)
		{
			$result=$db->sql_query("SELECT a.post_msg_id FROM ".ATTACHMENTS_TABLE." a LEFT JOIN ".FORUMS_TABLE." f ON (f.forum_id='{$row['forum_id']}') WHERE f.forumas='1' AND a.post_msg_id = '{$row['post_id']}' AND a.extension = 'torrent' LIMIT 1");
			$is_tracker=$db->sql_fetchfield('post_msg_id');
			if($is_tracker)
			{
				$decoded_message = 'Re: ' . $row['post_subject'];
			}
		}
		...
		foreach ($msg_list_ary as $email_template => $email_list)
		{
			foreach ($email_list as $addr)
			{
				...
				$messenger->assign_vars(array(
					...
					'U_STOP_WATCHING_TOPIC'	=> generate_board_url() . "/viewtopic.$phpEx?f=$forum_id&t=$topic_id&uid={$addr['user_id']}&unwatch=topic",
					'U_STOP_WATCHING_FORUM'	=> generate_board_url() . "/viewforum.$phpEx?f=$forum_id&uid={$addr['user_id']}&unwatch=forum",
				));
...
function submit_post($mode, $subject, $username, $topic_type, &$poll, &$data, $update_message = true, $update_search_index = true)
{
	...
	switch ($post_mode)
	{
		case 'post':
			...
			$sql_data[USERS_TABLE]['stat'][] = "user_lastpost_time = $current_time" . (($auth->acl_get('f_postcount', $data['forum_id']) && $post_approval) ? ', user_posts = user_posts + 1' : '').($data['forumas']==1 && $post_approval ? ', user_torrents = user_torrents + 1' : '');
			...
			if ($topic_type != POST_GLOBAL)
			{
				if ($post_approval)
				{
					...
					$sql_data[FORUMS_TABLE]['stat'][] = 'forum_torrents = forum_torrents + 1';
				}
		...
		case 'reply':
			...
			$sql_data[USERS_TABLE]['stat'][] = "user_lastpost_time = $current_time" . (($auth->acl_get('f_postcount', $data['forum_id']) && $post_approval) ? ', user_posts = user_posts + 1' : '').($data['forumas']==1 && $post_approval ? ', user_comments = user_comments + 1' : '');

			if ($post_approval && $topic_type != POST_GLOBAL)
			{
				$sql_data[FORUMS_TABLE]['stat'][] = 'forum_posts = forum_posts + 1';
				$sql_data[FORUMS_TABLE]['stat'][] = 'forum_comments = forum_comments + 1';
			}
		break;
		...
		case 'edit_first_post':
			...
			if (!$post_approval && $data['topic_approved'])
			{
				...
				set_config('num_torrents', $config['num_torrents'] - 1, true);
				set_config('num_comments', $config['num_comments'] - ($topic_row['topic_replies'] + 1), true);
			}
		...
		case 'edit_last_post':
			...
			if (!$post_approval && $data['post_approved'])
			{
				...
				set_config('num_comments', $config['num_comments'] - 1, true);
			}
	...
	if (!empty($data['attachment_data']) && $data['post_id'] && in_array($mode, array('post', 'reply', 'quote', 'edit')))
	{
		...
		{
			$sql = 'SELECT attach_id, filesize, physical_filename, extension
				FROM ' . ATTACHMENTS_TABLE . '
		...
		foreach ($data['attachment_data'] as $pos => $attach_row)
		{
			...
			else
			{
				...
				if($orphan_rows[$attach_row['attach_id']]['extension']=='torrent')
				{
					...
					if($mode=='edit')
					{
						$result=$db->sql_query("SELECT COUNT(*) thanks FROM ".TRACKER_THANKS_TABLE." WHERE post_id='{$data['post_id']}'");
						$thanks=my_int_val($db->sql_fetchfield('thanks'));
						if($thanks)
						{
							$attach_sql['thanks']=$thanks;
						}
					}
					$sql = 'UPDATE ' . TRACKER_TORRENTS_TABLE . ' SET ' . $db->sql_build_array('UPDATE', $attach_sql) . '
						WHERE id = ' . $attach_row['attach_id'];
					$db->sql_query($sql);
				}
			}
		}
	...
	if(@$data['forumas']==1 && ($is_cansetfree || $is_cansetreqratioupload))
	{
		$sql='UPDATE '. TRACKER_TORRENTS_TABLE ." SET free='". $torrent_free ."', req_ratio='". $torrent_reqratio ."', req_upload='". get_size_value(request_var('torrent_requploadv', 'b'), $torrent_requpload) ."' WHERE topic_id='{$data['topic_id']}'";
		$result=$db->sql_query($sql);
	}
...
function postscr_remote($data, &$error, $field, $i=0, $timeout=3)
{
	...
	$image_data=array();
	//From /includes/functions_convert.php
	...
	if (@$data['postscr_max_width'] || @$data['postscr_max_height'])
	{
	...
	if (@$data['postscr_min_width'] || @$data['postscr_min_height'])
	{
	...
	if($data['postscr_max_filesize'])
	{
		...
		if (!$postscr_file_size)
		{
			if (substr($data['remotelink'], 0, 4)=='http')
			{
				...
				{
					$postscr_file_size = intval($x['content-length']);
				}
			}
		}
		if($postscr_file_size && $postscr_file_size > $data['postscr_max_filesize'])
		{
	...
	return array($width, $height, $extension, image_type_to_mime_type($image_data[2]));
}
?>
