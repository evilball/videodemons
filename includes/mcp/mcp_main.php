...
{
	global $auth, $user, $db, $phpEx, $phpbb_root_path, $forum_id;


	$sql="SELECT forumas FROM ".FORUMS_TABLE." WHERE forum_id='{$forum_id}'";
	$result=$db->sql_query($sql);
	$row=$db->sql_fetchrow($result);
	$db->sql_freeresult($result);
	$forum_astracker=$row['forumas']==1 ? 1 : 0;
	if($forum_astracker && $action=='make_global')
	{
		$action='make_normal';
	}
...
function mcp_fork_topic($topic_ids)
{
	...
	$forum_astracker=$forum_astracker2=0;
	$result=$db->sql_query("SELECT forumas, forum_id FROM ".FORUMS_TABLE." WHERE forum_id IN('{$forum_id}', '{$to_forum_id}')");
	while($row=$db->sql_fetchrow($result))
	{
		if($row['forum_id']==$forum_id && $row['forumas']==1)
		{
			$forum_astracker=1;
		}
		if($row['forum_id']==$to_forum_id && $row['forumas']==1)
		{
			$forum_astracker2=1;
		}
	}
