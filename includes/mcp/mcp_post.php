...
function change_poster(&$post_info, $userdata)
{
	...
	if ($post_info['post_postcount'] && $post_info['post_approved'])
	{
		$sql = 'UPDATE ' . USERS_TABLE . '
			SET user_posts = user_posts - 1'.($post_info['forumas']==1 ? ($post_info['topic_first_post_id']==$post_info['topic_last_post_id'] ? ', user_torrents = user_torrents - 1' : ', user_comments = user_comments - 1') : '').'
			WHERE user_id = ' . $post_info['user_id'] .'
			AND user_posts > 0'.($post_info['forumas']==1 ? ($post_info['topic_first_post_id']==$post_info['topic_last_post_id'] ? ' AND user_torrents > 0' : ' AND user_comments > 0') : '');
		$db->sql_query($sql);
