	...
	*/
	function keyword_search($type, $fields, $terms, $sort_by_sql, $sort_key, $sort_dir, $sort_days, $ex_fid_ary, $m_approve_fid_ary, $topic_id, $author_ary, $author_name, &$id_ary, $start, $per_page)
	{
		...
		if($forum_astracker)
		{
			...
			{
				!isset($sql_array['FROM'][TOPICS_TABLE]) ? $sql_array['FROM'][TOPICS_TABLE]='t' : '';
				$sql_where[]='p.post_id != t.topic_first_post_id';
				...
				default:
					if ($type == 'posts')
					{
						$sql = 'SELECT COUNT(p.post_id) as total_results
							FROM ' . POSTS_TABLE . ' p' . ($firstpost_only || ($forum_astracker && $sql_sort[0]!='t' && ($t_search=='mycomment' || $t_search=='allcomment')) ? ', ' . TOPICS_TABLE . ' t ' : ' ') . "
							WHERE $sql_author
								$sql_topic_id
								$sql_firstpost
								$m_approve_fid_sql
