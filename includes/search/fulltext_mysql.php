...
class fulltext_mysql extends search_backend
{
	...
	*/
	...
	{
		global $config, $db, $forum_astracker, $ts_search;
		...
		if($forum_astracker)
		{
			if($ts_search=='comment')
			{
				$ts_sql_from.= !$join_topic ? ', '.TOPICS_TABLE . ' t ' : '';
				$sql_where_options.=' AND p.post_id != t.topic_first_post_id';
				$sql_where_options.=' AND p.topic_id = t.topic_id';
			}
			else if($ts_search=='torrent' || $ts_search=='tracker')
			{
				$ts_sql_from.=' LEFT JOIN '.TRACKER_TORRENTS_TABLE . ' tr ON (p.post_id = tr.post_msg_id)';
			}
  		}
		$sql = "SELECT $sql_select
			FROM $sql_from$sql_sort_table" . POSTS_TABLE . " p $ts_sql_from
	...
	*/
	...
	{
		global $config, $db, $forum_astracker, $t_search, $user, $user_id;
		...
		if($forum_astracker && ($t_search=='all' || $t_search=='my' || $t_search=='allcomment' || $t_search=='mycomment'))
		{
			if($t_search=='all' || $t_search=='my')
			{
				$firstpost_only=' AND p.post_id = t.topic_first_post_id';
				$type='topics';
			}
			else
			{
			...
			if($t_search=='all' || $t_search=='allcomment')
			{
				$author_ary[]=' p.poster_id > 1';
			}
			else if(($t_search=='my' || $t_search=='mycomment') && $user_id)
			{

				$author_ary[]="p.poster_id='$user_id'";
			}
			else
			{
				$author_ary[]="p.poster_id='{$user->data['user_id']}'";
			}
		}
		// generate a search_key from all the options to identify the results
		$search_key = md5(implode('#', array(
		...
		if($forum_astracker && ($t_search=='all' || $t_search=='my' || $t_search=='allcomment' || $t_search=='mycomment'))
		{
			...
			//$sql_author=($t_search=='all' || $t_search=='allcomment') ? ' p.poster_id > 1' : "p.poster_id='{$user->data['user_id']}'";

			$user_id=request_var('u', 0);
			if($t_search=='all' || $t_search=='allcomment')
			{
				$sql_author=' p.poster_id > 1';
			}
			else if(($t_search=='my' || $t_search=='mycomment') && $user_id)
			{
				$sql_author="p.poster_id='$user_id'";
			}
			else
			{
				$sql_author="p.poster_id='{$user->data['user_id']}'";
			}
		}
		...
		{
			$sql_addon=' AND p.topic_id=t.topic_id';
			/*if($t_search=='all')
			{
				$select=', MIN(t.topic_time) mtt';
				$sql_sort='mtt '.($sort_dir == 'a' ? ' ASC' : ' DESC');
			}
			else if($t_search=='my')
			{
				$sql_sort='t.forum_id ASC, '.$sql_sort;
			}*/
		}
		else
		{
		...
		if ($type == 'posts')
		{
			$sql = "SELECT {$calc_results}p.post_id
				FROM " . $sql_sort_table . POSTS_TABLE . ' p' . ($firstpost_only || ($forum_astracker && $sql_sort[0]!='t' && ($t_search=='mycomment' || $t_search=='allcomment')) ? ', ' . TOPICS_TABLE . ' t ' : ' ') . "
				WHERE $sql_author
					$sql_addon
					$sql_topic_id
					$sql_firstpost
		...
		else
		{
			$sql = "SELECT {$calc_results}t.topic_id
				FROM " . $sql_sort_table . TOPICS_TABLE . ' t, ' . POSTS_TABLE . " p".($forum_astracker && $t_search && in_array($t_search, array('all', 'new', 'active', 'uncomment', 'my')) ? ', '.TRACKER_TORRENTS_TABLE.' tr' : '')."
				WHERE $sql_author
					$sql_topic_id
					$sql_firstpost
					$m_approve_fid_sql
					...
					".($forum_astracker && $t_search && in_array($t_search, array('all', 'new', 'active', 'uncomment', 'my')) ? 'AND t.topic_id=tr.topic_id' : '')."
					$sql_sort_join
					$sql_time
				GROUP BY t.topic_id
				ORDER BY $sql_sort";
