...
class cache extends acm
{
	...
	*/
	function obtain_config()
	{
		...
		$cgp_configs=array('cookie_domain', 'cookie_name', 'cookie_path', 'cookie_secure', 'cgp_ttl', 'cgp_remove_sid', 'gzip_compress', 'cgp_multi_styles', 'default_style', 'cgp_places');
		$cgp_config_array=array();
		$cgp_extreme=false;
		...
		else
		{
			...
			while ($row = $db->sql_fetchrow($result))
			{
				...
				if($row['config_name']=='cgp_enabled' && $row['config_value']==2)
				{
					$cgp_extreme=true;
				}
				else if(in_array($row['config_name'], $cgp_configs))
				{
					$cgp_config_array[$row['config_name']]=$row['config_value'];
				}
		...
		if($cgp_extreme && $cgp_config_array['cgp_remove_sid']==1/* && $cgp_config_array['cgp_multi_styles']==0*/)
		{
			$this->put('_cgp_extreme', $cgp_config_array);
		}
		return $config;
	}
