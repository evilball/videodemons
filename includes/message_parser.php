...
class parse_message extends bbcode_firstpass
{
	...
	*/
	...
	{
		global $config, $auth, $user, $phpbb_root_path, $phpEx, $db, $forum_data;
		...
		$cfg = array();
		$t_attachments=1;
		if($forum_data['astracker'] && !$is_message)
		{
			//START MFU
			$as_poster = str_replace('fileupload', 'as_poster', $form_name);
			$image_as_poster=request_var($as_poster, 0);
			//END MFU
			$exists_attach=array('valid_tracker_torrents'=>0, 'valid_tracker_posters'=>0, 'valid_tracker_screenshots'=>0);
			$image_ext=array('jpg', 'jpeg', 'gif', 'png');
			foreach($this->attachment_data as $k => $v)
			{
				if($v['extension']=='torrent')
				{
					$exists_attach['valid_tracker_torrents']+=1;
				}
				else if(in_array($v['extension'], $image_ext))
				{
					if($v['i_poster']==1)
					{
						$exists_attach['valid_tracker_posters']+=1;
					}
					else
					{
						$exists_attach['valid_tracker_screenshots']+=1;
					}
				}
			}
			$ppkbb_max_torrents=1;

			$config['max_attachments'] = $ppkbb_max_torrents+$config['ppkbb_max_posters'][1]+$config['ppkbb_max_screenshots'][1];

			if($t_attachments && $upload_file/* && !$auth->acl_gets('a_', 'm_') && !$auth->acl_get('m_', $forum_id)*/)
			{
				$filename=trim($_FILES[$form_name]['name']);
				if (strpos($filename, '.') !== false)
				{
					...
					if($fileext=='torrent')
					{
						if($exists_attach['valid_tracker_torrents']+1 > $ppkbb_max_torrents)
						{
							$upload_file=false;
							$error['TMT']=$user->lang['MESS_TOOMANY_TORRENTS'];
						}
					}
					else if(in_array($fileext, $image_ext))
					{
						if(preg_match('/poster/i', $filename) || $image_as_poster)
						{
							if($config['ppkbb_max_posters'][1] && $exists_attach['valid_tracker_posters']+1 > $config['ppkbb_max_posters'][1])
							{
								$upload_file=false;
						...
						{
							if($config['ppkbb_max_screenshots'][1] && $exists_attach['valid_tracker_screenshots']+1 > $config['ppkbb_max_screenshots'][1])
							{
								$upload_file=false;
		...
		{
			if ($t_attachments && ($num_attachments < $cfg['max_attachments'] || $auth->acl_get('a_') || $auth->acl_get('m_', $forum_id)))
			{
				...
				{
					$filedata['i_poster']=0;
					if($forum_data['astracker'] && in_array($filedata['extension'], $image_ext))
					{
						if($image_as_poster)
						{
							if(!preg_match('/poster/i', $filedata['real_filename']))
							{
								$filedata['real_filename']='poster_'.$filedata['real_filename'];
							}
							$filedata['i_poster']=1;
						}
						else
					...
					$sql_ary = array(
						...
						'is_orphan'			=> 1,
						'i_width'			=> intval(@$filedata['i_width']),
						'i_height'		=> intval(@$filedata['i_height']),
						'i_poster'		=> $filedata['i_poster'],
						'i_external'		=> 0,
					...
					if($forum_data['astracker'] && $filedata['extension']=='torrent')
					{
						if(!function_exists('torrents_import'))
						{
							include_once($phpbb_root_path.'tracker/include/message_parser_add1.php');
						}

						$impres=torrents_import($filedata, $new_entry['attach_id']);
						if($impres!==true)
						{
							$error[]=$impres;
						}
						...
						{
							$filedata['filesize']=@filesize($phpbb_root_path.$config['upload_path'].'/'.$filedata['physical_filename']);

							$db->sql_query("UPDATE " . ATTACHMENTS_TABLE . " SET filesize='{$filedata['filesize']}' WHERE attach_id='{$new_entry['attach_id']}'");
						}
					}
				}
			}
				...
				if ($index !== false && !empty($this->attachment_data[$index]))
				{
					...
					if ($this->attachment_data[$index]['is_orphan'])
					{
						...
						if ($row)
						{
							...
							$db->sql_query('DELETE FROM ' . TRACKER_TORRENTS_TABLE . ' WHERE id = ' . (int) $this->attachment_data[$index]['attach_id']);
							$db->sql_query('DELETE FROM ' . TRACKER_FILES_TABLE . ' WHERE id = ' . (int) $this->attachment_data[$index]['attach_id']);
						}
					}
					else
					{
	...
	*/
	function get_submitted_attachment_data($check_user_id = false)
	{
		...
		if (sizeof($not_orphan))
		{
			...
			$sql = 'SELECT attach_id, is_orphan, real_filename, attach_comment, extension, filesize, download_count, physical_filename, thumbnail, i_width, i_height, i_poster, i_external
				FROM ' . ATTACHMENTS_TABLE . '
