...
class bbcode
{
	...
	*/
	function bbcode_tpl($tpl_name, $bbcode_id = -1, $skip_bitfield_check = false)
	{
		...
		if($config['ppkbb_spoiler_options'][3]==1)
		{
		...
		if (empty($bbcode_hardtpl))
		{
			...
			$bbcode_hardtpl = array(
				...
				'u_close'	=> '</span>',
				'img'		=> (!$config['ppkbb_spoiler_options'][4] && $config['ppkbb_spoiler_options'][5] ? '<a href="$1" rel="prettyPhotoSpoiler[0]">' : '').'<var title="$1" class="postImg" alt="' . $user->lang['IMAGE'] . '" />'.$img_wojs.'</var>'.(!$config['ppkbb_spoiler_options'][4] && $config['ppkbb_spoiler_options'][5] ? '</a>' : ''),
