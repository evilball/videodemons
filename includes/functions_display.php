...
function display_forums($root_data = '', $display_moderators = true, $return_moderators = false)
{
	...
	$chat_mess=$chat_ids=array();

	if($config['ppkbb_chat_enable'] && !$user->data['user_tracker_options'][1])
	{
		$sql='SELECT room, COUNT(*) mess, max(id) mid FROM '.PPKCHAT_MESSAGES_TABLE." WHERE to_user='0' AND room!='0' GROUP BY room";
		$result=$db->sql_query($sql, 60);
		while($row=$db->sql_fetchrow($result))
		{
			$chat_mess[$row['room']]=$row;
			$chat_ids[]=$row['mid'];
		}
		$db->sql_freeresult($result);

		$sql='SELECT room, COUNT(*) users FROM '.PPKCHAT_USERS_TABLE.' GROUP BY room';
		$result=$db->sql_query($sql, 60);
		while($row=$db->sql_fetchrow($result))
		{
			$chat_mess[$row['room']]=@array_merge($chat_mess[$row['room']], $row);
		}
		$db->sql_freeresult($result);

		if($chat_ids)
		{
			$sql='SELECT user_id, room, message, username, user_color, date FROM '.PPKCHAT_MESSAGES_TABLE." WHERE id IN('".implode("', '", $chat_ids)."')";
			$result=$db->sql_query($sql, 60);
			while($row=$db->sql_fetchrow($result))
			{
				$chat_mess[$row['room']]=array_merge($chat_mess[$row['room']], $row);
	...
	$sql_array['SELECT'] .= ', t.topic_replies, t.topic_id';
	...
	foreach ($forum_rows as $row)
	{
		...
		if ($row['parent_id'] == $root_data['forum_id'] && $row['forum_type'] == FORUM_CAT)
		{
			$template->assign_block_vars('forumrow', array(
				'FORUM_ASCHAT'=> $row['forumas']==2 ? true : false,
				'S_IS_CAT'				=> true,
				'FORUM_ID'				=> $row['forum_id'],
				...
				'U_VIEWFORUM'			=> $row['forumas']==2 && $row['forum_type'] != FORUM_CAT ? append_sid("{$phpbb_root_path}viewchat.$phpEx", 'f=' . $row['forum_id']) : append_sid("{$phpbb_root_path}viewforum.$phpEx", 'f=' . $row['forum_id']))
			);
			continue;
		}
		...
		if (isset($subforums[$forum_id]))
		{
			foreach ($subforums[$forum_id] as $subforum_id => $subforum_row)
			{
				...
				{
					$subforums_list[] = array(
						'link'		=> $row['forumas']==2 ? append_sid("{$phpbb_root_path}viewchat.$phpEx", 'f=' . $row['forum_id']) : append_sid("{$phpbb_root_path}viewforum.$phpEx", 'f=' . $subforum_id),
		...
		if ($row['forum_last_post_id'])
		{
			...
			$subject_post_url = append_sid("{$phpbb_root_path}viewtopic.$phpEx", 'f=' . $row['forum_id_last_post'] . '&amp;t=' . $row['topic_id']);

			$last_post_subject = $row['forum_last_post_subject'];
			$last_post_time = $user->format_date($row['forum_last_post_time']);
			$last_post_url = append_sid("{$phpbb_root_path}viewtopic.$phpEx", 'f=' . $row['forum_id_last_post'] . '&amp;t=' . $row['topic_id'].($viewtopic_page ? '&amp;start='.$viewtopic_page : '')). '#p' . $row['forum_last_post_id'];
		}
		else
		{
			$subject_post_url = $last_post_subject = $last_post_time = $last_post_url = '';
		}
		...
		if ($row['forum_type'] != FORUM_LINK)
		{
			$u_viewforum = $row['forumas']==2 ? append_sid("{$phpbb_root_path}viewchat.$phpEx", 'f=' . $row['forum_id']) : append_sid("{$phpbb_root_path}viewforum.$phpEx", 'f=' . $row['forum_id']);
		...
		if($row['forumas']==2)
		{
			if(@$chat_mess[$row['forum_id']])
			{
				$auth->acl_get('f_canviewchatm', $row['forum_id']) && $auth->acl_get('u_canviewchatm') ? '' : $chat_mess[$row['forum_id']]['message']='';
				$last_post_subject=isset($chat_mess[$row['forum_id']]['message']) ? htmlspecialchars(preg_replace('#\[/?(b|i|u|pre|samp|code|colou?r(=[^\[]+)?|size(=[^\[]+)?|noparse|url(=[^\[]+)?|s|q|blockquote|img(=[^\[]+)?)\]#', '', @$chat_mess[$row['forum_id']]['message'])) : '';
				$last_poster_full=@$chat_mess[$row['forum_id']]['username'] ? get_username_string('full', @$chat_mess[$row['forum_id']]['user_id'], $chat_mess[$row['forum_id']]['username'], $chat_mess[$row['forum_id']]['user_color']) : '';
				$row['forum_topics']=intval(@$chat_mess[$row['forum_id']]['users']);
				$post_click_count=intval(@$chat_mess[$row['forum_id']]['mess']);
				$last_post_time=$user->format_date(@$chat_mess[$row['forum_id']]['date']);
				$last_post_url=append_sid("{$phpbb_root_path}viewchat.$phpEx", 'f=' . $row['forum_id']);
			}
		}

		$template->assign_block_vars('forumrow', array(
			'FORUM_SHOW_INFO' => $row['forumas'],
			'FLAST_POST_SUBJECT'		=> $last_post_subject ? censor_text($last_post_subject) : '',
			'FORUM_ASCHAT'=> $row['forumas']==2 ? true : false,
			'U_LAST_SUBJECT'		=> $subject_post_url,
			...
			'LAST_POST_SUBJECT'		=> $last_post_subject ? $config['ppkbb_torrblock_width'][3] && utf8_strlen($last_post_subject) > $config['ppkbb_torrblock_width'][3] ? utf8_substr(censor_text($last_post_subject), 0, $config['ppkbb_torrblock_width'][3]).'...' : censor_text($last_post_subject) : '',
...
function generate_forum_nav(&$forum_data)
{
	...
	$template->assign_block_vars('navlinks', array(
		...
		'U_VIEW_FORUM'	=> $forum_data['forumas']!=2 ? append_sid("{$phpbb_root_path}viewforum.$phpEx", 'f=' . $forum_data['forum_id']) : append_sid("{$phpbb_root_path}viewchat.$phpEx", 'f=' . $forum_data['forum_id']))
	);
