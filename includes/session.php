...
//				$quadcheck = ($config['ip_check_bot'] && $this->data['user_type'] & USER_BOT) ? 4 : $config['ip_check'];
				...
				if ($u_ip === $s_ip && $s_browser === $u_browser && $s_forwarded_for === $u_forwarded_for && $referer_valid)
				{
					...
					if (!$session_expired)
					{
						...
						$this->data['user_tracker_options']=my_split_config($this->data['user_tracker_options'], 3, 'my_int_val');
						$this->data['is_registered'] ? '' : $config['ppkbb_tcclean_place']=array_fill(0, 6, 0);
						return true;
					}
				}
				else
...
class user extends session
{
	...
	*/
	function setup($lang_set = false, $style = false)
	{
		...
		if((!isset($this->data['is_registered']) || (isset($this->data['is_registered']) && !$this->data['is_registered'])) && $config['cgp_enabled'] && !$config['cgp_multi_styles'])
		{
			unset($_GET['style'], $_POST['style'], $_REQUEST['style'], $_COOKIE[$config['cookie_name'].'_style']);
		}


		if ((request_var('style', 0) || isset($_COOKIE[$config['cookie_name'].'_style']))/* && $auth->acl_get('a_styles')*/ && !defined('ADMIN_START'))
		{
			//global $SID, $_EXTRA_URL;
			if(isset($this->data['is_bot']) && $this->data['is_bot'])
			{
				unset($_GET['style'], $_POST['style'], $_REQUEST['style'], $_COOKIE[$config['cookie_name'].'_style']);
				$style=0;
			}
			...
			if(!$style)
			{
				$style=isset($_COOKIE[$config['cookie_name'].'_style']) ? intval($_COOKIE[$config['cookie_name'].'_style']) : 0;
			}
				else
			{
					$this->set_cookie('style', $style, time() + (86400*365));
				}

				//$this->data['user_style']=$style;
			//$SID .= '&amp;style=' . $style;
			...
			!$style ? $style=$config['default_style'] : '';
		}
		else
		{
			// Set up style
		...
		if (!$this->theme/* && $style == $this->data['user_style']*/)
		{
			...
			$style ? $this->set_cookie('style', $style, time()-3600) : '';
