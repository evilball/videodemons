...
function compose_pm($id, $mode, $action, $user_folders = array())
{
	...
	if ($action != 'edit')
	{
		...
		if ($config['allow_mass_pm'] && $auth->acl_get('u_masspm_group'))
		{
			...
			{
				$group_options .= '<option' . (($row['group_type'] == GROUP_SPECIAL) ? ' class="sep"' : '') . ' value="' . $row['group_id'] . '">' . (($row['group_type'] == GROUP_SPECIAL) || @$user->lang['G_' . $row['group_name']] ? $user->lang['G_' . $row['group_name']] : $row['group_name']) . '</option>';
			}
	...
	if ($submit || $preview || $refresh)
	{
		...
		{
			$message_parser->parse_attachments($key, $action, 0, $submit, $preview, $refresh, true);
		}
	...
	if($config['ppkbb_disable_fpquote'] && $action=='quotepost' && !$preview && !$refresh && !$submit)
	{
		$result=$db->sql_query("SELECT a.post_msg_id FROM ".ATTACHMENTS_TABLE." a LEFT JOIN ".FORUMS_TABLE." f ON (f.forum_id='{$post['forum_id']}') WHERE f.forumas='1' AND a.post_msg_id = '{$post_id}' AND a.extension = 'torrent' LIMIT 1");
		$is_tracker=$db->sql_fetchfield('post_msg_id');
		if($is_tracker)
		{
			$message_parser->message = $message_link . '[quote=&quot;' . $quote_username . '&quot;]' . $message_subject . "[/quote]\n";
		}
	}
		...
		foreach ($_types as $type)
		{
			if (isset($result[$type]) && $result[$type])
			{
				while ($row = $db->sql_fetchrow($result[$type]))
				{
					if ($type == 'g')
					{
						$row['name'] = ($row['group_type'] == GROUP_SPECIAL) || @$user->lang['G_' . $row['name']] ? $user->lang['G_' . $row['name']] : $row['name'];
					}
