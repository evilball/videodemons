...
class ucp_register
{
	...
	function main($id, $mode)
	{
		...
		if($config['ppkbb_ipreg_countrestrict'][0])
		{
			$sql = "SELECT count(user_id) ip_count FROM ".USERS_TABLE." WHERE user_ip = '".$db->sql_escape($user->ip)."'";
			$result = $db->sql_query($sql);
			$ip_count=$db->sql_fetchfield('ip_count');
			if(intval($ip_count) >= $config['ppkbb_ipreg_countrestrict'][0])
			{
				trigger_error(sprintf($user->lang['TRIPREG_COUNTRESTRICT'], $config['ppkbb_ipreg_countrestrict'][0]));
			}
		}
		...
		if ($submit)
		{
			...
			if (!sizeof($error))
			{
				...
				$config['ppkbb_ipreg_countrestrict'][1] ? $user_row['user_uploaded']=$config['ppkbb_ipreg_countrestrict'][1] : '';
