...
class acm
{
	...
	*/
	function sql_load($query, $cache_static='')
	{
		// Remove extra spaces and tabs
		$query = preg_replace('/[\n\r\s\t]+/', ' ', $query);

		if (($rowset = $this->_read('sql_' . md5($cache_static ? $cache_static : $query))) === false)
		{
			return false;
		}
	...
	* Save sql query
	*/
	function sql_save($query, &$query_result, $ttl, $cache_static='')
	{
		global $db;
		...
		if ($this->_write('sql_' . md5($cache_static ? $cache_static : $query), $this->sql_rowset[$query_id], $ttl + time(), $query))
		{
	...
	* Removes/unlinks file
	*/
	function my_remove_cache($filename, $type='sql', $check = false, $mass='')
	{
		global $phpbb_root_path, $phpEx;
		...
		if($mass)
		{
			$od=@opendir($cache_dir);
			if($od)
			{
				while(($f=readdir($od))!==false)
				{
					if(preg_match("/^{$type}_{$mass}\.{$phpEx}$/", $f))
					{
						@unlink("{$cache_dir}{$f}");
					}
				}
