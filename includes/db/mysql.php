...
class dbal_mysql extends dbal
{
	...
	* @access	public
	*/
	function sql_query($query = '', $cache_ttl = 0, $cache_static='')
	{
		if ($query != '')
		{
			global $cache;
			...
			$this->query_result = ($cache_ttl && method_exists($cache, 'sql_load')) ? $cache->sql_load($query, $cache_static) : false;
			...
			if ($this->query_result === false)
			{
				...
				if ($cache_ttl && method_exists($cache, 'sql_save'))
				{
					...
					$cache->sql_save($query, $this->query_result, $cache_ttl, $cache_static);
				}
