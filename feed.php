...
function feed_format_date2($time)
{
	static $zone_offset;
	static $offset_string;
	...
	return gmdate("Y-m-d H:i:s", $time + $zone_offset) . $offset_string;
}
/**
...
class phpbb_feed_factory
{
	...
	*/
	function init($mode, $forum_id, $topic_id)
	{
		...
		{
			case 'torrents':
				return new phpbb_feed_torrents();
			break;

			case 'comments':
				return new phpbb_feed_comments();
			break;
			case 'forums':
...
class phpbb_feed_torrents extends phpbb_feed_base
{
	function set_keys()
	{
		global $config;
		...
		$this->set('updated',	'topic_time');

		$this->set('enable_bbcode',		'enable_bbcode');
		$this->set('enable_smilies',	'enable_smilies');
		$this->set('enable_magic_url',	'enable_magic_url');

		$this->num_items = (int) $config['ppkbb_feed_torrents_limit'];
	}
	...
	function get_sql()
	{
		global $db, $config, $forum_id;

		$forum_ids = array();

		if (!$forum_id)
		{
			if(!$config['ppkbb_feed_torrents'])
			{
				return false;
			}
		}
		...
		{
			if(!$config['ppkbb_feed_ftorrents'])
			{
				return false;
			}
		...
		$sql_addon=$forum_ids ? " AND f.forum_id IN('".(implode("', '", $forum_ids))."')" : ($forum_id ? " AND f.forum_id='{$forum_id}'" : '');

		$post_ids = $config['ppkbb_feed_enblist']=array();

		$not_in_fid = ($config['ppkbb_feed_enblist']) ? ' AND ' . $db->sql_in_set('t.forum_id', $config['ppkbb_feed_enblist'], $config['ppkbb_feed_trueenblist'] ? true : false) : '';

		// Search for topics in last x days
		$config['ppkbb_feed_torrtime'] ? '' : $config['ppkbb_feed_torrtime']=$this->sort_days;
		$last_post_time_sql = ' AND '.($config['ppkbb_feed_torrents_sort'] ? 't.topic_time' : 'tr.added').' > ' . (time() - $config['ppkbb_feed_torrtime'] * 24 * 3600);

		// Last x topics from all forums, with first post from topic...
		$sql = 'SELECT t.topic_first_post_id
			FROM ' . TOPICS_TABLE . ' t, '.FORUMS_TABLE.' f, '.TRACKER_TORRENTS_TABLE." tr
			WHERE f.forumas='1'{$sql_addon} AND f.forum_id=t.forum_id AND tr.topic_id=t.topic_id AND tr.post_msg_id=t.topic_first_post_id AND t.topic_approved = 1
				AND t.topic_moved_id = 0" .
				$not_in_fid .
				$last_post_time_sql . '
			ORDER BY '.($config['ppkbb_feed_torrents_sort'] ? 't.topic_time DESC' : 'tr.added DESC');
		$result = $db->sql_query_limit($sql, $this->num_items);
		...
		$this->sql = array(
			'SELECT'	=> 'f.forum_id, f.forum_password, f.forum_name, f.forum_topics, f.forum_posts, f.parent_id, f.left_id, f.right_id,
							...
							u.username, u.user_id, u.user_email, u.user_colour, tr.id, tr.forb, tr.size, tr.seeders, tr.leechers, tr.times_completed',
			'FROM'		=> array(
				TOPICS_TABLE	=> 't',
				FORUMS_TABLE	=> 'f',
				POSTS_TABLE		=> 'p',
			...
			'WHERE'		=> $db->sql_in_set('p.post_id', $post_ids) .'
								...
								AND tr.post_msg_id=p.post_id
								AND u.user_id = p.poster_id',
			'ORDER_BY'	=> $config['ppkbb_feed_torrents_sort'] ? 't.topic_time DESC' : 'tr.added DESC',
		);
	...
	function adjust_item(&$item_row, &$row)
	{
		...
		if ($config['feed_item_statistics'])
		{
			...
			$item_row['statistics'] = $user->lang['POSTED'] . ' ' . $user->lang['POST_BY_AUTHOR'] . ' ' . $user_link . ' - ' . feed_format_date2($row['topic_time']). ' - ' . $user->lang['REPLIES'] . ' ' . $row['topic_replies'] . ' - ' . $user->lang['VIEWS'] . ' ' . $row['topic_views'];
		}
	}
}

class phpbb_feed_comments extends phpbb_feed_base
{
	function set_keys()
	{
		global $config;
		...
		$this->num_items = (int) $config['ppkbb_feed_comments_limit'];
	}
	...
	function get_sql()
	{
		global $db, $config, $forum_id, $topic_id, $user;

		$forum_ids = array();

		if ($forum_id && $topic_id)
		{
			if(!$config['ppkbb_feed_tcomments'])
			{
				return false;
			}
		}
		...
		{
			if(!$config['ppkbb_feed_fcomments'])
			{
				return false;
			}
			// Determine forum childs...
		...
		else if(!$forum_id && !$topic_id)
		{
			if(!$config['ppkbb_feed_comments'])
			{
				return false;
			}
		}
		...
		// Search for topics in last x days
		$config['ppkbb_feed_commtime'] ? '' : $config['ppkbb_feed_commtime']=$this->sort_days;
		$last_post_time_sql = ' AND t.topic_last_post_time > ' . (time() - $config['ppkbb_feed_commtime'] * 24 * 3600);

		// Last x topics from all forums, with first post from topic...
		$sql = 'SELECT p.post_id
			FROM ' . POSTS_TABLE . ' p, '. TOPICS_TABLE . ' t, '.FORUMS_TABLE." f
			WHERE f.forumas='1'{$sql_addon}".($topic_id ? " AND p.topic_id='{$topic_id}'" : '').' AND f.forum_id=t.forum_id AND t.topic_id=p.topic_id AND p.post_id!=t.topic_first_post_id AND t.topic_approved = 1 AND p.post_approved = 1
				AND t.topic_moved_id = 0' .
				$not_in_fid .
				$last_post_time_sql . '
		...
		$this->sql = array(
			'SELECT'	=> 'f.forum_id, f.forum_password, f.forum_name, f.forum_topics, f.forum_posts, f.parent_id, f.left_id, f.right_id,
							t.topic_id, t.topic_title, t.topic_poster, t.topic_first_poster_name, t.topic_replies, t.topic_views, t.topic_time,
							p.post_time, p.post_id, p.post_text, p.bbcode_bitfield, p.bbcode_uid, p.enable_bbcode, p.enable_smilies, p.enable_magic_url,
	...
	function adjust_item(&$item_row, &$row)
	{
		...
		if ($config['feed_item_statistics'])
		{
			...
			$item_row['statistics'] = $user->lang['POSTED'] . ' ' . $user->lang['POST_BY_AUTHOR'] . ' ' . $user_link . ' - ' . feed_format_date2($row['post_time']). ' - ' . $user->lang['REPLIES'] . ' ' . $row['topic_replies'] . ' - ' . $user->lang['VIEWS'] . ' ' . $row['topic_views'];
		}
	}
}
?>
