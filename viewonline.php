...
// Get user list
$sql = 'SELECT u.user_id, u.username, u.username_clean, u.user_type, u.user_colour, s.session_id, s.session_time, s.session_page, s.session_ip, s.session_browser, s.session_viewonline, s.session_forum_id'.($config['ppkbb_forumid_remove'] ? ', t.forum_id, t.topic_title, t.topic_id' : '').'
	FROM ' . USERS_TABLE . ' u, ' . SESSIONS_TABLE . ' s'.($config['ppkbb_forumid_remove'] ? ' LEFT JOIN '.TOPICS_TABLE.' t ON(s.session_topic_id=t.topic_id)' : '').'
...
while ($row = $db->sql_fetchrow($result))
{
	...
	switch ($on_page[1])
	{
		...
		case 'viewtopic':
			...
			$topic_id=0;
			if($config['ppkbb_forumid_remove'] && $row['forum_id'])
			{
			...
			{
				$location = '';
				$location_url = $topic_id ? append_sid("{$phpbb_root_path}viewtopic.$phpEx", 'f=' . $forum_id.'&amp;t='.$topic_id) : append_sid("{$phpbb_root_path}viewforum.$phpEx", 'f=' . $forum_id);
				...
				switch ($on_page[1])
				{
					...
					case 'viewtopic':
						$location = $topic_id ? sprintf($user->lang['READING_TOPIC_FULL'], $row['topic_title'], $forum_data[$forum_id]['forum_name']) : sprintf($user->lang['READING_TOPIC'], $forum_data[$forum_id]['forum_name']);
					break;
...
while ($row = $db->sql_fetchrow($result))
{
	...
	{
		$legend .= (($legend != '') ? ', ' : '') . '<a style="color:#' . $row['group_colour'] . '" href="' . append_sid("{$phpbb_root_path}memberlist.$phpEx", 'mode=group&amp;g=' . $row['group_id']) . '">' . (($row['group_type'] == GROUP_SPECIAL) || @$user->lang['G_' . $row['group_name']] ? $user->lang['G_' . $row['group_name']] : $row['group_name']) . '</a>';
	}
}
$db->sql_freeresult($result);
