...
if (!$topic_data)
{
	...
	if ($post_id && $topic_id)
	{
		redirect(append_sid("{$phpbb_root_path}viewtopic.$phpEx",  (($forum_id) ? "f=$forum_id&amp;" : '') . "t=$topic_id"));
	}
...
if (!class_exists('CGP') && $config['cgp_enabled'] && !$config['ppkbb_cgp_places'][3])
{
	include($phpbb_root_path . 'includes/cache_guests_pages.' . $phpEx);
}

if (defined('CGP_ENABLED') && !$config['ppkbb_cgp_places'][3])
{
...
if($topic_data['forumas']==2)
{
	trigger_error(defined('IN_PORTAL') ? 'CHAT_PORTAL_ERROR' : 'CHAT_INDEX_ERROR');
}
$dt=time();
$posting_page=$torrent_id=$s_hasnovote=0;
$forum_image=$topic_data['forum_image'] ? $topic_data['forum_image'] : 0;
$forum_astracker=$topic_data['forumas']==1 && $topic_data['topic_type']!=POST_GLOBAL ? 1 : 0;
!$topic_data['forum_first_post_show'] ? $topic_data['topic_first_post_show']=0 : '';
$topic_data['forum_first_post_show']==1 ? $topic_data['topic_first_post_show']=1 : '';
$start && !$topic_data['topic_first_post_show'] ? $forum_astracker=0 : '';
$torrents_cleanup=$user_trsl_update=$torrents_remote=$torrents_hashes=$update_tstatus=array();
if($forum_astracker)
{
	$i2=$i3=0;
	$sql='SELECT
		...
		u.username,
		u.user_colour
			FROM '.TRACKER_TORRENTS_TABLE ." tr LEFT JOIN ". POSTS_TABLE ." p ON (p.topic_id=tr.topic_id AND p.post_id=tr.post_msg_id) LEFT JOIN ".TRACKER_THANKS_TABLE." tt ON (tt.post_id=tr.post_msg_id AND tt.user_id='{$user->data['user_id']}') LEFT JOIN ".USERS_TABLE." u ON (tr.forb_user_id=u.user_id) WHERE tr.topic_id='{$topic_data['topic_id']}'";
	$result=$db->sql_query($sql);
	while($row=$db->sql_fetchrow($result))
	{
		if($row['torrent_id'])
		{
			...
			$config['ppkbb_tccleanup_interval'] && $config['ppkbb_tcclean_place'][3] && $dt - $row['lastcleanup'] > $config['ppkbb_tccleanup_interval'] ? $torrents_cleanup[$row['torrent_id']]=$row['torrent_id'] : '';
			$config['ppkbb_tcenable_rannounces'][0] && $config['ppkbb_tcenable_rannounces'][3] && $row['forb'] < 1 ? $torrents_hashes[$row['torrent_id']]=$row['info_hash'] : '';
		}
	}
	$db->sql_freeresult($result);
	if(!$torrent_id)
...
if($config['ppkbb_chat_enable'] && !$config['ppkbb_chat_display'][3] && $config['ppkbb_index_chat'] && !defined('IN_PORTAL') && !defined('IN_CHAT') && !$user->data['user_tracker_options'][1])
{
...
$template->assign_vars(array(
	...
	'TR_THANKS_IMG' 		=> $user->img('icon_say_thanks', '', '', '', 'src'),
	'TOTOP_IMG' 			=> $user->img('icon_back_top', 'BACK_TO_TOP'),
	'SUBJECT_TEXTLENGTH'		=> $config['ppkbb_subject_textlength'],

	'S_CHAT_INDEX' => defined('SHOW_CHAT') ? true : false,
	'POSTERS_TABLE_WIDTH' => $forum_astracker ? $config['ppkbb_torrblock_width'][8]+$config['ppkbb_torrblock_width'][1] : '',
	'SCREENSHOTS_TABLE_WIDTH' => $forum_astracker ? $config['ppkbb_torrblock_width'][10]+$config['ppkbb_torrblock_width'][1] : '',
	'S_TRACKER_BLOCKS' => $forum_astracker && array_sum($config['ppkbb_torr_blocks'])  ? true : false,
...
if(!$user->data['is_registered'])
{
	$is_candowntorr = $config['ppkbb_tcguests_enabled'][0] && $auth->acl_get('u_candowntorr') && $auth->acl_get('f_candowntorr', $forum_id) ? 1 : 0;
		$config['ppkbb_tcguests_enabled'][0] && $config['ppkbb_tctrestricts_options'][2] && $torrents[$torrent_id]['size'] < $config['ppkbb_tctrestricts_options'][2] ? $is_candowntorr=1 : '';
...
$is_canusethanks = ($user->data['is_registered'] && $auth->acl_get('u_canusethanks') && $auth->acl_get('f_canusethanks', $forum_id)) ? 1 : 0;
}
...
if (!empty($topic_data['poll_start']))
{
	$sql = 'SELECT o.*, p.bbcode_bitfield, p.bbcode_uid
		...
		ORDER BY ".(!$forum_astracker ? 'o.poll_option_id' : 'o.poll_option_total DESC');
	$result = $db->sql_query($sql);
	...
	if($forum_astracker && (!$torrent_id || !$is_candowntorr))
	{
		$s_can_vote=false;
	}
	$s_display_results = (!$s_can_vote || ($s_can_vote && sizeof($cur_voted_id)) || $view == 'viewpoll') ? true : false;
	//$s_display_results ? $s_can_vote=false : '';
	...
	$template->assign_vars(array(
		...
		'S_HAS_VOTE' => $s_hasnovote || !$config['ppkbb_torr_blocks'][5] ? false : true,
	...
	if (!isset($user_cache[$poster_id]))
	{
		if ($poster_id == ANONYMOUS)
		{
			$user_cache[$poster_id] = array(
				...
				'warnings'			=> 0,
				'allow_pm'			=> 0,
				'user_torrents'				=> $row['user_torrents'],
				...
				'user_ratio'				=> get_ratio($row['user_uploaded'], $row['user_downloaded'], $config['ppkbb_tcratio_start'], $row['user_bonus']),
				'user_fromthanks'				=> $row['user_fromthanks_count'],
				'user_tothanks'				=> $row['user_tothanks_count'],
				'user_tracker_data'				=> $row['user_tracker_data'],
			);
...
$torrents_attach=array();

// Pull attachment data
if (sizeof($attach_list))
{
	if ($is_candownload || ($forum_astracker && ($is_candowntorr || $is_candownpostscr)))
	{
		$sql = 'SELECT *
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			if($forum_astracker && $row['post_msg_id'] == $topic_data['topic_first_post_id'])
			{
				if($row['extension']=='torrent')
				{
					if($is_candowntorr && $torrent_id==$row['attach_id'])
					{
						$torrents_attach['torrent'][]=$row;
						$to_user=$row['poster_id'];
					}
				...
				else if(in_array($row['extension'], $image_ext))
				{
					if($is_candownpostscr)
					{
						if($row['i_poster']==1)
						{
							$torrents_attach['poster'][]=$row;
						}
						else
						{
							$torrents_attach['screenshot'][]=$row;
						}
					}
					$row['skip_attachment']=1;
				}
		...
		if (!sizeof($attachments) && !$torrents_attach)
		{
...
for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
{
	...
	$postrow=array();
	if($forum_astracker && $torrent_id && $row['post_id'] == $topic_data['topic_first_post_id'])
	{
		include_once($phpbb_root_path.'tracker/include/viewtopic_add1.'.$phpEx);
	}
	else
	{
		$torrent_stat=$assign_vars=$torrent_action=array();
	}
	...
	$postrow = array_merge($postrow, array(
		'U_TORRENTS'			=> $user_cache[$poster_id]['user_torrents'],
		'U_COMMENTS'			=> $user_cache[$poster_id]['user_comments'],
		'U_UPLOADS'			=> $user_cache[$poster_id]['user_uploaded'],
		'U_DOWNLOADS'			=> $user_cache[$poster_id]['user_downloaded'],
		'U_RATIO'		=> get_ratio_alias($user_cache[$poster_id]['user_ratio']),
		'S_HAS_TORRENT'	=> $torrent_id && !$start ? true : false,
		'U_FROMTHANKS'			=> $user_cache[$poster_id]['user_fromthanks'],
		'U_TOTHANKS'			=> $user_cache[$poster_id]['user_tothanks'],
		...
		'U_MINI_POST'		=> append_sid("{$phpbb_root_path}viewtopic.$phpEx", "f={$forum_id}&amp;t={$topic_id}&amp;p={$row['post_id']}".($start ? "&amp;start={$start}" : '')) . '#p' . $row['post_id'],
	...
	if($forum_astracker && $torrent_id && $row['post_id'] == $topic_data['topic_first_post_id'])
	{
		include_once($phpbb_root_path.'tracker/include/viewtopic_add2.'.$phpEx);
	}
...
{
	include_once($phpbb_root_path.'tracker/include/viewtopic_add_cron.'.$phpEx);
}
page_footer();
