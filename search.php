...
$fp=request_var('fp', 0);
if($keywords && $fp && $config['search_type']=='fulltext_mysql' && !preg_match('/^&quot;(.*)&quot;$/', $keywords))
{
	$keywords='"'.$keywords.'"';
}
...
$valid_tsearch=array('uncomment'=>1, 'active'=>1, 'all'=>1, 'new'=>1, 'my'=>1, 'allcomment'=>1, 'newcomment'=>1, 'mycomment'=>1);
$valid_tssearch=array('tracker'=>1, 'torrent'=>1, 'comment'=>1);//, 'all'=>1, 'forum'=>1
$t_search=request_var('torrent', '');
$ts_search=request_var('tracker_search', '');
$torrents_cleanup=$update_tstatus=$torrents_hashes=$torrents_remote=array();
if($t_search && isset($valid_tsearch[$t_search]) && $user->data['is_registered'])
{
	$forum_astracker=1;
	in_array($t_search, array('all', 'new', 'active', 'uncomment', 'my')) ? $show_results='topics' : $show_results='posts';
}
else
{
	$t_search=0;
...
$is_cansearchtr=$auth->acl_get('u_cansearchtr') ? 1 : 0;

if($ts_search && isset($valid_tssearch[$ts_search]) && $is_cansearchtr)
{
	$forum_astracker=1;
	if($ts_search=='torrent')
	{
		if($search_fields!='titleonly')
		{
			$search_fields='firstpost';
		}
	}
	else if($ts_search=='tracker')
	{
		$search_fields='all';
	}
	else if($ts_search=='comment')
	{
...
if(($forum_astracker && $show_results=='topics') || ($is_cansearchtr && !$submit && !$t_search && !$ts_search && !in_array($sort_key, array('sz', 'ts', 'tl', 'tc', 'th', 'tf'))))
{
...
if ($keywords || $author || $author_id || $search_id || $submit)
{
	...
	if($forum_astracker)
	{
		if($t_search)
		{
			$result=$db->sql_query('SELECT forum_id FROM '. FORUMS_TABLE ." WHERE forumas!='1'");
			while($row=$db->sql_fetchrow($result))
			{
				$ex_fid_ary[]=$row['forum_id'];
			}
			$db->sql_freeresult($result);
			if(in_array($t_search, array('all', 'new', 'active', 'uncomment', 'my')))
			{
				$show_results='topics';
			}
			else
		...
		if($ts_search)
		{
			...
			$result=$db->sql_query('SELECT forum_id FROM '. FORUMS_TABLE ." WHERE {$ts_addon}");
			while($row=$db->sql_fetchrow($result))
			{
				$ex_fid_ary[]=$row['forum_id'];
			}
	...
	$sql = 'SELECT f.forum_id, f.forum_name, f.parent_id, f.forum_type, f.right_id, f.forum_password, f.forum_flags, fa.user_id, f.enable_indexing
		FROM ' . FORUMS_TABLE . ' f
	...
	while ($row = $db->sql_fetchrow($result))
	{
		...
		if (!$row['enable_indexing'] && !empty($keywords))
		{
			$ex_fid_ary[] = (int) $row['forum_id'];
			continue;
		}
	...
	$sort_by_sql = array('a' => 'u.username_clean', 't' => (($show_results == 'posts') ? 'p.post_time' : ($forum_astracker && $t_search ? ($t_search!='seeders' && $t_search!='myseeders' ? 't.topic_time' : 'tr.leechers'.($config['ppkbb_tcenable_rannounces'][0] ? '+tr.rem_leechers' : '')) : 't.topic_last_post_time')), 'f' => 'f.forum_id', 'i' => 't.topic_title', 's' => (($show_results == 'posts') ? 'p.post_subject' : 't.topic_title'));
	if($forum_astracker && $show_results=='topics' && (in_array($t_search, array('all', 'new', 'active', 'uncomment', 'my'))))
	{
	...
	if ($search_id)
	{
		switch ($search_id)
		{
			...
			case 'unanswered':
				...
				else
				{
					$sql = 'SELECT DISTINCT ' . $sort_by_sql[$sort_key] . ", p.topic_id
						FROM $sort_join" . POSTS_TABLE . ' p, ' . TOPICS_TABLE . " t".($forum_astracker && $t_search ? ', '.TRACKER_TORRENTS_TABLE . ' tr' : '')."
						WHERE t.topic_replies = 0
							AND t.topic_moved_id = 0
							AND p.topic_id = t.topic_id
							".($forum_astracker && $t_search ? ' AND p.post_id = tr.post_msg_id' : '')."
							$last_post_time
							$m_approve_fid_sql
							" . ((sizeof($ex_fid_ary)) ? ' AND ' . $db->sql_in_set('p.forum_id', $ex_fid_ary, true) : '') . "
						$sql_sort";
			...
			case 'newposts':
				...
				if($forum_astracker && $t_search)
				{
					if($t_search=='newcomment')
					{
						$sort_dir='d';
						$sort_by_sql['t']='p.post_time';
					}
					else if($t_search=='new')
					{
						$sort_dir='d';
						$sort_by_sql['t']='t.forum_id ASC, t.topic_time';
					}
				}
				...
				if($forum_astracker && $t_search)
				{
					...
					else if($t_search=='newcomment')
					{
						$sql_addon=' AND p.post_id != t.topic_first_post_id AND t.topic_id=p.topic_id';
						$show_results='posts';
					}
					else
					{
				...
				{
					$sql = 'SELECT p.post_id
						FROM ' . POSTS_TABLE . ' p'.($forum_astracker && $t_search ? ', '. TOPICS_TABLE.' t ' : '').'
						WHERE p.post_time > ' . $user->data['user_lastvisit'] . $sql_addon."
							$m_approve_fid_sql
							" . ((sizeof($ex_fid_ary)) ? ' AND ' . $db->sql_in_set('p.forum_id', $ex_fid_ary, true) : '') . "
						$sql_sort";
					$field = 'post_id';
				...
				{
					if($forum_astracker && $t_search=='new')
					{
						$sql = 'SELECT t.topic_id
							FROM ' . TOPICS_TABLE . ' t
							WHERE t.topic_time > ' . $user->data['user_lastvisit'].$sql_addon . '
								AND t.topic_moved_id = 0
								' . str_replace(array('p.', 'post_'), array('t.', 'topic_'), $m_approve_fid_sql) . '
								' . ((sizeof($ex_fid_ary)) ? 'AND ' . $db->sql_in_set('t.forum_id', $ex_fid_ary, true) : '') . "
							$sql_sort";
...
*/
	...
	if (sizeof($id_ary))
	{
		...
		if($forum_astracker && $t_search)
		{
			...
			{
				$sql_where.=' AND t.forum_id > 0';
			}
			else
			{
				$sql_where.=' AND p.forum_id > 0';
			}
		}
	}
	...
	if($forum_astracker && $t_search)
	{
		...
		switch ($t_search)
		{
			case 'uncomment':
				...
				$l_search_title=$user->lang['SEARCH_UNANSWERED_TORRENT'];
				break;
			case 'active':
				$search_id='active_topics';
				$l_search_title=$user->lang['SEARCH_ACTIVE_TORRENT'];
				break;
			case 'all':
				$search_id='egosearch';
				$l_search_title=$user->lang['SEARCH_ALL_TORRENT'];
				break;
			case 'new':
				$search_id='newposts';
				$l_search_title=$user->lang['SEARCH_NEW_TORRENT'];
				break;
			case 'my':
				$search_id='egosearch';
				if(!$user_id)
				{
					$l_search_title=$user->lang['SEARCH_SELF_TORRENT'];
				}
				else
				{
					$l_search_title=$user->lang['SEARCH_USER_TORRENT'];
					$author_id=0;
				}
				break;
			case 'allcomment':
				$search_id='egosearch';
				$l_search_title=$user->lang['SEARCH_ALL_COMMENT'];
				break;
			case 'mycomment':
				$search_id='egosearch';
				if(!$user_id)
				{
					$l_search_title=$user->lang['SEARCH_SELF_COMMENT'];
				}
				else
				{
					$l_search_title=$user->lang['SEARCH_USER_COMMENT'];
				}
				break;
			case 'newcomment':
				$search_id='newposts';
				$l_search_title=$user->lang['SEARCH_NEW_COMMENT'];
				break;
			default:
				break;
		}
	...
	$u_search = append_sid("{$phpbb_root_path}search.$phpEx", $u_sort_param . $u_show_results . "&amp;torrent={$t_search}&amp;tracker_search={$ts_search}".($user_id ? "&amp;u={$user_id}" : ''));
	...
	if ($sql_where)
	{
		...
		else
		{
			...
			$sql_select = 't.*, f.forum_id, f.forum_name, f.forum_image';
			...
			if($forum_astracker && $show_results == 'topics' && ($t_search || in_array($ts_search, array('tracker', 'comment', 'torrent'))))
			{
				$sql_from .= ' LEFT JOIN ' . TRACKER_TORRENTS_TABLE . ' tr ON (t.topic_id = tr.topic_id)';
				$sql_select .= ', tr.id torrent_id,
		...
		if ($show_results == 'topics')
		{
			...
			while ($row = $db->sql_fetchrow($result))
			{
				...
				if($forum_astracker)
				{
					...
					if(isset($row['torrent_id']) && $row['torrent_id'])
					{
						$config['ppkbb_tccleanup_interval'] && $config['ppkbb_tcclean_place'][1] && $dt - $row['lastcleanup'] > $config['ppkbb_tccleanup_interval'] ? $torrents_cleanup[$row['torrent_id']]=$row['torrent_id'] : '';
						$config['ppkbb_tcenable_rannounces'][0] && $config['ppkbb_tcenable_rannounces'][2] && $row['forb'] < 1 ? $torrents_hashes[$row['torrent_id']]=$row['info_hash'] : '';
					}
				}
		...
		foreach ($rowset as $row)
		{
			...
			if ($show_results == 'topics')
			{
				...
				if(!$no_torrent)
				{
					...
					$torrent_link=1;
					if($forum_astracker && ($t_search || in_array($ts_search, array('tracker', 'comment', 'torrent'))))
					{
						...
						if($is_candowntorr)
						{
							if(!isset($is_canusefree[$forum_id]))
							{
								$is_canusefree[$forum_id] = ($auth->acl_get('u_canusefree') && $auth->acl_get('f_canusefree', $forum_id)) ? 1 : 0;
								$is_canusebonus[$forum_id] = ($auth->acl_get('u_canusebonus') && $auth->acl_get('f_canusebonus', $forum_id)) ? 1 : 0;
								$is_canskiprcheck[$forum_id] = $user->data['is_registered'] && $user->data['user_id']!=$row['topic_poster'] ? (($auth->acl_get('u_canskiprcheck') && $auth->acl_get('f_canskiprcheck', $forum_id)) ? 1 : 0) : 1;
								$is_canskiprequpload[$forum_id] = $user->data['is_registered'] && $user->data['user_id']!=$row['topic_poster'] ? (($auth->acl_get('u_canskiprequpload') && $auth->acl_get('f_canskiprequpload', $forum_id)) ? 1 : 0) : 1;
								$is_canskipreqratio[$forum_id] = $user->data['is_registered'] && $user->data['user_id']!=$row['topic_poster'] ? (($auth->acl_get('u_canskipreqratio') && $auth->acl_get('f_canskipreqratio', $forum_id)) ? 1 : 0) : 1;
							}

							$t_elapsed = intval(($dt - $row['added']) / 3600);
							$t_wait=-1;
							if(!$is_canskiprcheck[$forum_id] && $config['ppkbb_tcwait_time'])
							{
							...
							$t_wait2=-1;
							if(!$is_canskiprcheck[$forum_id] && $config['ppkbb_tcwait_time2'])
							{
							...
							if(!$is_canskiprequpload[$forum_id] && $row['req_upload'])
							{
							...
							if(!$is_canskipreqratio[$forum_id] && $row['req_ratio']!=0.000)
							{
							...
							$freetorr_percent=$is_canusefree[$forum_id] ? $row['free'] : 0;
							...
							$magnet_src_link='';
							if($torrent_link && (($config['ppkbb_torrent_magnetlink'][2] && $user->data['is_registered']) || (!$user->data['is_registered'] && $config['ppkbb_torrent_gmagnetlink'][2] && $config['ppkbb_tcguests_enabled'][0])))
							{
					...
					if(!isset($is_candownpostscr[$forum_id]))
					{
						$is_candownpostscr[$forum_id]=$auth->acl_get('u_candownpostscr') && $auth->acl_get('f_candownpostscr', $forum_id) ? 1 : 0;
					}
				}
				...
				$tpl_ary = array(
					'TOPIC_FORUM_IMG'		=> $row['forum_image'] ? $phpbb_root_path . $row['forum_image'] : '',
	...
	if($forum_astracker)
	{
		include_once($phpbb_root_path.'tracker/include/search_add_cron.'.$phpEx);
	}
	...
	if($forum_astracker && ($t_search || in_array($ts_search, array('tracker', 'comment', 'torrent'))))
	{
		$template->set_filenames(array(
			'body' => 'search_results_tracker.html')
		);
	}
	else
	{
...
// Search forum
$s_forums = '';
$sql = 'SELECT f.forum_id, f.forum_name, f.parent_id, f.forum_type, f.left_id, f.right_id, f.forum_password, f.enable_indexing, f.forumas, fa.user_id
	FROM ' . FORUMS_TABLE . ' f
	LEFT JOIN ' . FORUMS_ACCESS_TABLE . " fa ON (fa.forum_id = f.forum_id
		AND fa.session_id = '" . $db->sql_escape($user->session_id) . "')
