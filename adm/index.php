...
function adm_page_header($page_title)
{
	...
	$template->assign_vars(array(
		...
		'TRACKER_ADDIT_CSS' => "{$phpbb_root_path}tracker/cssjs.{$phpEx}?type=css&amp;css_set=1".(!$config['ppkbb_cssjs_cache'] ? '&amp;no_cache=1' : ''),
		'TRACKER_ADDIT_JS' => "{$phpbb_root_path}tracker/cssjs.{$phpEx}?type=js&amp;js_set=1".(!$config['ppkbb_cssjs_cache'] ? '&amp;no_cache=1' : ''),
...
*/
function build_cfg_template($tpl_type, $key, &$new, $config_key, $vars, $map=false)
{
	global $user, $module, $phpbb_root_path, $phpEx;
		...
		case 'array':
			if($map)
			{
				if($map[0])
				{
					...
					!is_array($new[$config_key]) ? $new[$config_key]=my_split_config($new[$config_key], $map[0], $map[1], $map[2]) : '';
					for($i=0;$i<$map[0];$i++)
					{
						if(isset($map['field']))
						{
									...
									switch($field_type[0])
									{
										...
										case 'multi':
											...
											for($i2=0;$i2<$field_count;$i2++)
											{
												if($i2)
												{
													...
													$tpl.='<option value="' . $field_value[0] . '"'.(($new[$config_key][$i]==$field_value[0]) || (isset($map['default'][$i]) && $map['default'][$i]!==false && $new[$config_key][$i]==='') ? ' selected="selected"' : '').'>'.(isset($user->lang[$vars['lang'].'_'.$field_value[1]]) ? $user->lang[$vars['lang'].'_'.$field_value[1]] : (isset($user->lang[$field_value[1]]) ? $user->lang[$field_value[1]] : $field_value[1])).'</option>';
												}
											}
											$tpl.='</select>';
										break;
										...
										case 'checkbox':
											...
											for($i2=0;$i2<$field_count;$i2++)
											{
												...
												{
													$field_value=explode('=', $field_type[$i2]);
													$tpl.='<label><input class="radio" type="'.($field_type[0]=='radio' ? 'radio' : 'checkbox').'" name="' . $name .($map[0] > 1 ?  '['.$i.']' : '').'" value="' . $field_value[0] . '"'.(($new[$config_key][$i]==$field_value[0]) || (isset($map['default'][$i]) && $map['default'][$i]!==false && $new[$config_key][$i]==='') ? ' checked="checked"' : '').' /> '.(isset($user->lang[$vars['lang'].'_'.$field_value[1]]) ? $user->lang[$vars['lang'].'_'.$field_value[1]] : (isset($user->lang[$field_value[1]]) ? $user->lang[$field_value[1]] : $field_value[1])).'</label>';
												}
											}
										break;
										...
										case 'time':
											...
											$td = new timedelta();
											$tpl .= '<input id="' . $key .($map[0] > 1 ?  '['.$i.']' : '').'" type="text"' . ($field_type[1] ? ' size="' . $field_type[1] . '"' : '') . ' maxlength="' . (($field_type[2]) ? $field_type[2] : 255) . '" name="' . $name .($map[0] > 1 ?  '['.$i.']' : '').'" value="' . (isset($map['default'][$i]) && $map['default'][$i]!==false && $new[$config_key][$i]==='' ? $map['default'][$i] : $new[$config_key][$i]) . '" />&nbsp;<select name="' . $name . 'v'.($map[0] > 1 ? '['.$i.']' : '').'">'.select_time_value('s').'</select>&nbsp;['.$td->spelldelta(0, $new[$config_key][$i]).']';
										break;
										...
										default:
											$tpl .= '<input id="' . $key . '['.$i.']" type="text"' . (($size) ? ' size="' . $size . '"' : '') . ' maxlength="' . (($maxlength) ? $maxlength : 255) . '" name="' . $name .($map[0] > 1 ?  '['.$i.']' : '').'" value="' . (isset($map['default'][$i]) && $map['default'][$i]!==false && $new[$config_key][$i]==='' ? $map['default'][$i] : $new[$config_key][$i]) . '" />';
										break;
									}
						...
						else
						{
							...
							$tpl .= '<input id="' . $key . '['.$i.']" type="text"' . (($size) ? ' size="' . $size . '"' : '') . ' maxlength="' . (($maxlength) ? $maxlength : 255) . '" name="' . $name . '['.$i.']" value="' . (isset($map['default']) && $map['default']!==false && $new[$config_key][$i]==='' ? $map['default'] : $new[$config_key][$i]) . '" />'.(isset($map['append'][$i]) && isset($user->lang[$map['append'][$i]]) ? '&nbsp;'.$user->lang[$map['append'][$i]] : '').'<br />';
						}
					}
				}
			}
...
function validate_config_vars($config_vars, &$cfg_array, &$error)
{
	...
	foreach ($config_vars as $config_name => $config_definition)
	{
		...
		switch ($validator[$type])
		{
			...
			case 'array':
				if(stripos($config_name, 'board3_ppkbb3cker_')===0)
				{
					$map_config_name=preg_replace('/\d+$/', '', $config_name);
				}
				else
				{
					$map_config_name=$config_name;
				}
				$validator_min=0;
				$validator_max=3000;

				isset($config_map[$map_config_name]) && is_array($cfg_array[$config_name]) ? $cfg_array[$config_name]=implode(($config_map[$map_config_name][2] ? $config_map[$map_config_name][2] : ' '), $cfg_array[$config_name]) : '';
